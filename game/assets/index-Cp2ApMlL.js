(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const o of s.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function t(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function r(i){if(i.ep)return;i.ep=!0;const s=t(i);fetch(i.href,s)}})();const pe=40,Se=30,ni=50,Ke=24;function ti(e){return e==="&"?"&amp;":e==="<"?"&lt;":e===">"?"&gt;":e}class ii{constructor(){this.charW=0,this.charH=0,this.gridH=Se,this.resizeHandlers=[],this.debounceTimer=null,this.pre=document.createElement("pre"),this.pre.className="game-screen",this.pre.dataset.gridCols=String(pe),this.pre.dataset.gridRows=String(Se),document.body.appendChild(this.pre);const n=()=>{this.measureChar(),this.applyScale()};Promise.all([document.fonts.ready,document.fonts.load(`${Ke}px "Share Tech Mono"`).catch(()=>null)]).then(n),document.fonts.addEventListener("loadingdone",n),window.addEventListener("resize",()=>{this.debounceTimer!==null&&clearTimeout(this.debounceTimer),this.debounceTimer=setTimeout(()=>this.applyScale(),100)})}measureChar(){const n=document.createElement("span");n.style.fontFamily="'Share Tech Mono', monospace",n.style.fontSize=`${Ke}px`,n.style.lineHeight="1em",n.style.position="absolute",n.style.visibility="hidden",n.textContent="M",document.body.appendChild(n);const t=n.getBoundingClientRect();document.body.removeChild(n),this.charW=t.width,this.charH=t.height}applyScale(){if(this.charW<=0||this.charH<=0)return;const n=Math.min(window.innerWidth/(pe*this.charW),window.innerHeight/(Se*this.charH)),t=Math.max(Se,Math.min(ni,Math.floor(window.innerHeight/(this.charH*n))));if(this.pre.style.fontSize=`${Ke*n}px`,this.pre.style.width=`${pe*this.charW*n}px`,t!==this.gridH){this.gridH=t,this.pre.dataset.gridRows=String(t);for(const r of this.resizeHandlers)r(pe,t)}}onResize(n){this.resizeHandlers.push(n)}drawBuffer(n){const t=[];for(const r of n){let i="";for(const s of r){const o=s.fg!=="transparent"?`fg-${s.fg}`:"",a=s.bg!=="transparent"?`bg-${s.bg}`:"",l=o&&a?`${o} ${a}`:o||a,c=l?` class="${l}"`:"";i+=`<span${c}>${ti(s.char)}</span>`}t.push(i)}this.pre.innerHTML=t.join(`
...
`)}}onAction(n){this.actionHandlers.push(n)}onTap(n){this.tapHandlers.push(n)}onCharInput(n){this.charInputHandlers.push(n)}connect(){this.keyListener=n=>{if(si.has(n.key)&&n.preventDefault(),oi.has(n.key))for(const r of this.charInputHandlers.slice())r(n.key);if(n.key==="Backspace"||n.key==="Delete"){for(const r of this.charInputHandlers.slice())r("\b");return}const t=ri[n.key];if(t)for(const r of this.actionHandlers.slice())r(t)},document.addEventListener("keydown",this.keyListener),this.pointerDownListener=n=>{n.preventDefault(),this.pointerStartMap.set(n.pointerId,{startX:n.clientX,startY:n.clientY}),this.activePointers.add(n.pointerId),this.logDebug(`DOWN id=${n.pointerId} (${Math.round(n.clientX)},${Math.round(n.clientY)}) type=${n.pointerType} active=${this.activePointers.size}`)},this.pointerUpListener=n=>{const t=this.pointerStartMap.get(n.pointerId),r=this.activePointers.size;if(this.activePointers.delete(n.pointerId),this.pointerStartMap.delete(n.pointerId),!t){this.logDebug(`UP id=${n.pointerId} NO START`);return}const i=n.clientX-t.startX,s=n.clientY-t.startY,o=Math.abs(i),a=Math.abs(s);if(o<20&&a<20)if(r>=2){this.logDebug(`UP id=${n.pointerId} 2-finger tap -> BACK`),this.activePointers.clear(),this.pointerStartMap.clear();for(const l of this.actionHandlers.slice())l("BACK")}else{const l=this.getRectInfo(),c=this.getGridCoords(t.startX,t.startY);if(c){this.logDebug(`TAP (${Math.round(t.startX)},${Math.round(t.startY)}) rect=${l} -> col=${c.col} row=${c.row} h=${this.tapHandlers.length}`);for(const h of this.tapHandlers.slice())h(c.col,c.row)}else this.logDebug(`TAP (${Math.round(t.startX)},${Math.round(t.startY)}) rect=${l} -> OOB`)}else{let l;o>=a?l=i>0?"RIGHT":"LEFT":l=s>0?"DOWN":"UP",this.logDebug(`SWIPE dx=${Math.round(i)} dy=${Math.round(s)} -> ${l}`);for(const c of this.actionHandlers.slice())c(l)}},this.pointerCancelListener=n=>{this.logDebug(`CANCEL id=${n.pointerId}`),this.activePointers.delete(n.pointerId),this.pointerStartMap.delete(n.pointerId)},window.addEventListener("pointerdown",this.pointerDownListener),window.addEventListener("pointerup",this.pointerUpListener),window.addEventListener("pointercancel",this.pointerCancelListener)}disconnect(){this.keyListener&&(document.removeEventListener("keydown",this.keyListener),this.keyListener=null),this.pointerDownListener&&(window.removeEventListener("pointerdown",this.pointerDownListener),this.pointerDownListener=null),this.pointerUpListener&&(window.removeEventListener("pointerup",this.pointerUpListener),this.pointerUpListener=null),this.pointerCancelListener&&(window.removeEventListener("pointercancel",this.pointerCancelListener),this.pointerCancelListener=null),this.pointerStartMap.clear(),this.activePointers.clear()}getRectInfo(){const n=document.querySelector(".game-screen");if(!n)return"NO PRE";const t=n.getBoundingClientRect();return`L${Math.round(t.left)},T${Math.round(t.top)},R${Math.round(t.right)},B${Math.round(t.bottom)}`}getGridCoords(n,t){const r=document.querySelector(".game-screen");if(!r)return null;const i=r.getBoundingClientRect(),s=parseInt(r.dataset.gridCols??"1"),o=parseInt(r.dataset.gridRows??"1");if(!s||!o||!i.width||!i.height)return null;const a=Math.floor((n-i.left)/(i.width/s)),l=Math.floor((t-i.top)/(i.height/o));return a<0||a>=s||l<0||l>=o?null:{col:a,row:l}}}function f(e,n,t,r,i,s){if(n<0||n>=e.length)return;const o=e[n];for(let a=0;a<r.length;a++){const l=t+a;l>=0&&l<o.length&&(o[l]={char:r[a],fg:i,bg:s})}}function k(e,n,t,r,i){if(n<0||n>=e.length)return;const s=e[n].length,o=Math.max(0,Math.floor((s-t.length)/2));f(e,n,o,t,r,i)}function ln(e,n){const t=e.split(/\s+/).filter(Boolean),r=[];let i="";for(const s of t)i.length===0?i=s:i.length+1+s.length<=n?i+=" "+s:(r.push(i),i=s);return i.length>0&&r.push(i),r}const bn=["UNTITLED","SPACE GAME"],li=4,ci=3,hi="- An ASCII space adventure -",di=11,_n=16;class vn{constructor(n,t,r,i){this.cursorIdx=0,this.activated=!1,this.items=[{label:"NEW GAME",action:()=>{this.activated=!0,i()}}],t.environment==="terminal"&&this.items.push({label:"QUIT",action:()=>{console.log("[MainMenu] Quitting…"),process.exit(0)}}),n.onAction(s=>{this.activated||(s==="UP"?this.cursorIdx=(this.cursorIdx-1+this.items.length)%this.items.length:s==="DOWN"?this.cursorIdx=(this.cursorIdx+1)%this.items.length:s==="SELECT"&&this.items[this.cursorIdx].action())}),n.onTap&&n.onTap((s,o)=>{if(!this.activated){for(let a=0;a<this.items.length;a++)if(o===_n+a){this.cursorIdx=a,this.items[a].action();return}}})}update(n){}render(n){const t=n.length,r=t>0?n[0].length:0;for(let o=0;o<t;o++)for(let a=0;a<r;a++)n[o][a]={char:" ",fg:"black",bg:"black"};for(let o=0;o<bn.length;o++)k(n,li+o*ci,bn[o],"bright-cyan","black");k(n,di,hi,"white","black");const i=this.items.reduce((o,a)=>Math.max(o,a.label.length+2),0),s=Math.max(0,Math.floor((r-i)/2));for(let o=0;o<this.items.length;o++){const a=_n+o;if(a>=t)continue;const l=o===this.cursorIdx,c=l?"> ":"  ",h=l?"bright-green":"white";f(n,a,s,c+this.items[o].label,h,"black")}}}let tn=null;function ui(e){tn=e}function U(){if(tn===null)throw new Error("World not initialised — call initWorld() before accessing world data");return tn}function De(e){return U().systems.find(n=>n.id===e)}function P(e){return U().destinations.find(n=>n.id===e)}function cn(e){return U().routes.filter(n=>n.from===e||n.to===e)}function dt(e){return U().drives.find(n=>n.id===e)}function pi(e){return U().storyBeats.filter(n=>n.trigger===e)}function fi(){return U().settings}function ut(e){return U().ships.find(n=>n.id===e)}function Oe(e,n){return U().routes.find(t=>t.from===e&&t.to===n||t.from===n&&t.to===e)}function ee(e){return U().commodities.find(n=>n.id===e)}function mi(){return U().commodities}function gi(){return U().systems.filter(e=>e.playerKnowledge==="public")}function yi(e){return e.reduce((n,t)=>{const r=ee(t.commodityId);return n+t.qty*((r==null?void 0:r.weightKg)??0)},0)}const C=3;function pt(e,n){return n?e-2:e}function bi(e){return e.toLocaleString("en-US")}class ve{constructor(n,t){this.buttonRanges=null,this.footerRow=-1,this.context=n,this.player=t}render(n,t){const r=n.length,i=r>0?n[0].length:0;this.footerRow=-1,this.buttonRanges=null,t.showHeader&&(this.renderHeaderRow0(n,i,t.systemLabel),this.renderHeaderRow1(n,i,t.destinationLabel)),t.showFooter&&(this.renderFooter(n,r,i,t.navOptions),this.footerRow=r-1)}renderHeaderRow0(n,t,r){const i=r!==void 0?r??"":(()=>{const c=De(this.player.systemId);return c?c.name.toUpperCase():this.player.systemId.toUpperCase()})(),s="::";f(n,0,0,s,"bright-black","black"),f(n,0,s.length,i,"bright-cyan","black");const a=t-s.length-i.length-10;let l=s.length+i.length;for(let c=0;c<a;c++)n[0][l+c]={char:":",fg:"bright-black",bg:"black"};l+=a,f(n,0,l,"[M]","white","black"),l+=3,f(n,0,l," MENU","white","black"),l+=5,f(n,0,l,"::","bright-black","black")}renderHeaderRow1(n,t,r){const i=r!==void 0?r??"":(()=>{const h=this.player.destinationId?P(this.player.destinationId):null;return h?h.name.toUpperCase():"IN SPACE"})(),s=bi(this.player.credits),o=s.length+5,a="::";f(n,1,0,a,"bright-black","black"),f(n,1,a.length,i,"cyan","black");const l=t-a.length-i.length-o;let c=a.length+i.length;for(let h=0;h<l;h++)n[1][c+h]={char:":",fg:"bright-black",bg:"black"};c+=l,f(n,1,c,s,"green","black"),c+=s.length,f(n,1,c," CR","white","black"),c+=3,f(n,1,c,"::","bright-black","black")}renderFooter(n,t,r,i){const s=t-1,o=[];if(i.length===0){for(let l=0;l<r;l++)n[s][l]={char:":",fg:"bright-black",bg:"black"};this.buttonRanges=[];return}f(n,s,0,"::","bright-black","black");let a=2;for(let l=0;l<i.length;l++){l>0&&(f(n,s,a,"::","bright-black","black"),a+=2);const c=i[l],h=`[${l+1}]`,d=` ${c.label}`,u=a;f(n,s,a,h,"white","black"),a+=h.length,f(n,s,a,d,"white","black"),a+=d.length,o.push({id:c.id,startCol:u,endCol:a})}for(let l=a;l<r;l++)n[s][l]={char:":",fg:"bright-black",bg:"black"};this.buttonRanges=o}hitTestNav(n,t){if(this.footerRow<0||this.buttonRanges===null||t!==this.footerRow)return null;for(const r of this.buttonRanges)if(n>=r.startCol&&n<r.endCol)return r.id;return null}}const _i=3,wn=5;class vi{constructor(n,t,r,i){this.activated=!1,this.pageIndex=0,this.onContinue=i,this.chrome=new ve(t,r);const o=pi("game-start")[0].text.split(`

`),a=o[0].trim();let l;/^YEAR\s+\d{4}$/.test(a)?(this.yearHeader=a,l=o.slice(1)):(this.yearHeader="",l=o);const c=[];for(let h=0;h<l.length;h++){const d=l[h].replace(/\n/g," "),u=ln(d,36);h>0&&c.push(""),c.push(...u)}this.bodyLines=c,n.onAction(h=>{this.activated||(h==="SELECT"?(this.activated=!0,this.onContinue()):h==="LEFT"?this.pageIndex>0&&this.pageIndex--:h==="RIGHT"&&this.pageIndex++)}),n.onTap&&n.onTap((h,d)=>{this.activated||(this.activated=!0,this.onContinue())})}update(n){}render(n){const t=n.length,r=t>0?n[0].length:0;for(let d=0;d<t;d++)for(let u=0;u<r;u++)n[d][u]={char:" ",fg:"black",bg:"black"};if(this.chrome.render(n,{showHeader:!0,showFooter:!0,navOptions:[]}),this.yearHeader){const d=Math.max(0,Math.floor((r-this.yearHeader.length)/2));f(n,_i,d,this.yearHeader,"bright-yellow","black")}const s=t-3-wn,o=Math.max(1,Math.ceil(this.bodyLines.length/s));this.pageIndex>=o&&(this.pageIndex=o-1);const a=o>1,l=this.pageIndex*s,c=Math.min(l+s,this.bodyLines.length);let h=wn;for(let d=l;d<c;d++){const u=this.bodyLines[d];u!==""&&f(n,h,2,u,"white","black"),h++}if(a){const d=`< ${this.pageIndex+1}/${o} >`,u=r-9;f(n,t-3,u,d,"bright-black","black")}}}const ft=5,fe=10;class we{constructor(n,t,r,i,s,o,a=[],l=null){this.activeTabIdx=0,this.cursorIdx=-1,this.pageIndex=0,this.lastPageCount=1,this.activated=!1,this.modal=null,this.title=n,this._staticItems=t,this.tabs=l,this.context=s,this.player=o,this.chrome=new ve(s,o),this.navOptions=r,this.infoLines=a,this.itemStartRow=l!==null?C+5:C+3+a.length,i.onCharInput&&i.onCharInput(c=>{this.modal!==null&&this.modal.handleCharInput(c)}),i.onAction(c=>{if(this.modal!==null){this.modal.handleAction(c);return}this.activated||(c==="UP"?this.moveCursor(-1):c==="DOWN"?this.moveCursor(1):c==="LEFT"&&this.tabs!==null?(this.activeTabIdx=Math.max(0,this.activeTabIdx-1),this.resetCursor()):c==="RIGHT"&&this.tabs!==null?(this.activeTabIdx=Math.min(this.tabs.length-1,this.activeTabIdx+1),this.resetCursor()):c==="PAGE_UP"?(this.pageIndex=(this.pageIndex-1+this.lastPageCount)%this.lastPageCount,this.resetCursor()):c==="PAGE_DOWN"?(this.pageIndex=(this.pageIndex+1)%this.lastPageCount,this.resetCursor()):c==="SELECT"?this.activateCurrent():this.handleNavAction(c))}),this.resetCursor(),i.onTap&&i.onTap((c,h)=>{if(this.modal!==null){this.modal.handleTap(c,h);return}if(this.activated)return;const d=this.chrome.hitTestNav(c,h);if(d!==null){this.handleNavTap(d);return}if(this.tabs!==null&&h===C+3){let p=3;for(let m=0;m<this.tabs.length;m++){const y=this.tabs[m].label.length+2;if(c>=p&&c<p+y){this.activeTabIdx=m,this.resetCursor();return}p+=y+1}}const u=this.rowToVisibleItemIndex(h);u!==null&&!this.items[u].disabled&&(this.cursorIdx=u,this.activateCurrent())})}get items(){var n;return this.tabs!==null?((n=this.tabs[this.activeTabIdx])==null?void 0:n.items)??[]:this._staticItems}moveCursor(n){const t=this.items,r=t.length;if(r===0)return;const i=this.cursorIdx===-1?n>0?r-1:0:this.cursorIdx;for(let s=0;s<r;s++){const o=((i+n*(s+1))%r+r)%r;if(!t[o].disabled){this.cursorIdx=o;return}}}activateCurrent(){if(this.items.length===0||this.cursorIdx===-1)return;const n=this.items[this.cursorIdx];n.disabled||(this.activated=!0,n.action())}rowToVisibleItemIndex(n){var i;let t=this.itemStartRow;const r=this.items;for(let s=0;s<r.length;s++){const o=1+(((i=r[s].details)==null?void 0:i.length)??0);if(n>=t&&n<t+o)return s;t+=o}return null}resetCursor(){const n=this.items;for(let t=0;t<n.length;t++)if(!n[t].disabled){this.cursorIdx=t;return}this.cursorIdx=-1}handleNavAction(n){}handleNavTap(n){}openModal(n){this.modal=n}closeModal(){this.modal=null}buildChromeConfig(){return{showHeader:!0,showFooter:!0,navOptions:this.navOptions}}update(n){}render(n){var A,E;const t=n.length,r=t>0?n[0].length:0;for(let w=0;w<t;w++)for(let b=0;b<r;b++)n[w][b]={char:" ",fg:"black",bg:"black"};const i=this.buildChromeConfig();this.chrome.render(n,i),f(n,C,2,this.title,"bright-white","black"),f(n,C+1,2,"'".repeat(this.title.length),"bright-black","black");for(let w=0;w<this.infoLines.length;w++)f(n,C+2+w,2,this.infoLines[w],"bright-black","black");if(this.tabs!==null){const w=C+3;let b=2;n[w][b]={char:"|",fg:"bright-black",bg:"black"},b++;for(let S=0;S<this.tabs.length;S++){const R=S===this.activeTabIdx,W=` ${this.tabs[S].label} `,N=R?"black":"white",M=R?"green":"black";for(const D of W)b<r&&(n[w][b]={char:D,fg:N,bg:M}),b++;b<r&&(n[w][b]={char:"|",fg:"bright-black",bg:"black"}),b++}}const o=pt(t,i.showFooter)-1,a=o-this.itemStartRow,l=this.items,c=l.map(w=>{var b;return 1+(((b=w.details)==null?void 0:b.length)??0)}),d=c.reduce((w,b)=>w+b,0)>a,u=d?a-1:a,p=[];let m=[],y=0;for(let w=0;w<c.length;w++)y+c[w]>u?(m.length>0&&p.push(m),m=[w],y=c[w]):(m.push(w),y+=c[w]);m.length>0&&p.push(m),this.lastPageCount=Math.max(1,p.length),this.pageIndex>=this.lastPageCount&&(this.pageIndex=this.lastPageCount-1);const g=p[this.pageIndex]??[];let v=this.itemStartRow;for(const w of g){const b=l[w],S=w===this.cursorIdx,R=b.disabled?"bright-black":S?"bright-green":"white",W=b.infoFg??R,N=r-4;if(b.icon!==void 0){const M=b.icon.length;if(f(n,v,2,S?">":" ",R,"black"),f(n,v,3,b.icon,b.iconFg??R,"black"),b.info!==void 0){const H=b.info.length,te=Math.max(0,N-1-M-2-H-1),X=b.label.length>te?b.label.slice(0,te):b.label,de=Math.max(1,N-1-M-X.length-2-H);f(n,v,3+M,X+" ",R,"black"),f(n,v,3+M+X.length+1,".".repeat(de),"bright-black","black"),f(n,v,3+M+X.length+1+de+1,b.info,W,"black")}else f(n,v,3+M,b.label.slice(0,N-1-M),R,"black");for(let H=0;H<(((A=b.details)==null?void 0:A.length)??0);H++)v+1+H<=o&&f(n,v+1+H,2,("  "+b.details[H]).slice(0,N),"bright-black","black")}else if(b.info!==void 0){const M=S?"> ":"  ",D=Math.max(1,N-2-b.label.length-2-b.info.length);f(n,v,2,M+b.label+" ",R,"black"),f(n,v,2+M.length+b.label.length+1,".".repeat(D),"bright-black","black"),f(n,v,2+M.length+b.label.length+1+D+1,b.info,W,"black")}else if(b.details!==void 0&&b.details.length>0){f(n,v,2,((S?"> ":"  ")+b.label).slice(0,N),R,"black");for(let D=0;D<b.details.length;D++)v+1+D<=o&&f(n,v+1+D,2,("  "+b.details[D]).slice(0,N),"bright-black","black")}else f(n,v,2,((S?"> ":"  ")+b.label).slice(0,N),R,"black");v+=1+(((E=b.details)==null?void 0:E.length)??0)}if(d){const w=`${this.pageIndex+1}/${this.lastPageCount}`,b=o;f(n,b,0,"|<|","white","black");const S=Math.floor((r-w.length)/2);f(n,b,S,w,"bright-black","black"),f(n,b,r-3,"|>|","white","black")}this.modal!==null&&this.modal.render(n)}}const wi=30;class rn{constructor(n){this.focus="field",this.replaceNextDigit=!0,this.confirmRect=null,this.cancelRect=null,this.formDef=n,this.value=Math.max(n.field.min,Math.min(n.field.max,n.field.initialValue))}handleAction(n){const{field:t}=this.formDef;if(n==="BACK"){this.formDef.onCancel();return}if(this.focus==="field"){if(n==="UP"){this.value=Math.min(t.max,this.value+1);return}if(n==="DOWN"){this.value=Math.max(t.min,this.value-1);return}if(n==="RIGHT"){this.value=Math.min(t.max,this.value+10);return}if(n==="LEFT"){this.value=Math.max(t.min,this.value-10);return}}if(n==="TAB"){this.focus==="field"?this.focus="confirm":this.focus==="confirm"?this.focus="cancel":this.focus="field";return}n==="SELECT"&&(this.focus==="cancel"?this.formDef.onCancel():this.formDef.onConfirm(this.value))}handleCharInput(n){if(this.focus!=="field")return;const{field:t}=this.formDef;if(n==="\b")this.value=Math.floor(this.value/10),this.value===0&&(this.replaceNextDigit=!0);else if(n>="0"&&n<="9"){const r=parseInt(n,10);this.replaceNextDigit?(this.value=Math.max(t.min,Math.min(t.max,r)),this.replaceNextDigit=!1):this.value=Math.min(t.max,this.value*10+r)}}handleTap(n,t){this.confirmRect&&t===this.confirmRect.row&&n>=this.confirmRect.col&&n<this.confirmRect.col+this.confirmRect.width?this.formDef.onConfirm(this.value):this.cancelRect&&t===this.cancelRect.row&&n>=this.cancelRect.col&&n<this.cancelRect.col+this.cancelRect.width&&this.formDef.onCancel()}render(n){const t=n.length,r=t>0?n[0].length:0,{title:i,field:s,derivedRows:o,confirmLabel:a}=this.formDef,l=o.length,c=8+l,h=wi,d=Math.floor((r-h)/2),u=Math.floor((t-c)/2);for(let x=0;x<c;x++)for(let $=0;$<h;$++){const Y=u+x,ue=d+$;Y>=0&&Y<t&&ue>=0&&ue<r&&(n[Y][ue]={char:" ",fg:"white",bg:"black"})}const p=(x,$,Y)=>{x>=0&&x<t&&$>=0&&$<r&&(n[x][$]={char:Y,fg:"white",bg:"black"})};p(u,d,"+"),p(u,d+h-1,"+");for(let x=1;x<h-1;x++)p(u,d+x,"-");p(u+c-1,d,"+"),p(u+c-1,d+h-1,"+");for(let x=1;x<h-1;x++)p(u+c-1,d+x,"-");for(let x=1;x<c-1;x++)p(u+x,d,"|"),p(u+x,d+h-1,"|");const m=h-2,y=u+1,g=d+1+Math.floor((m-i.length)/2);f(n,y,g,i,"bright-white","black"),f(n,u+2,g,"'".repeat(i.length),"bright-black","black");const v=[s.label,...o.map(x=>x.label)],A=Math.max(...v.map(x=>x.length)),E=d+1+A+3,w=u+4,b=this.focus==="field";f(n,w,d+1,s.label.padEnd(A)+" : ","white","black");const S=this.value.toString().padStart(5);f(n,w,E,S,b?"black":"white",b?"green":"black");for(let x=0;x<l;x++){const $=o[x],Y=u+5+x,ue=$.compute(this.value);f(n,Y,d+1,$.label.padEnd(A)+" : ","white","black"),f(n,Y,E,ue,"white","black")}const R=u+4+l+2,W=`[ ${a} ]`,N="[ CANCEL ]",M=3,D=W.length+M+N.length,H=Math.floor((m-D)/2),te=d+1+H,X=te+W.length+M;this.confirmRect={col:te,row:R,width:W.length},this.cancelRect={col:X,row:R,width:N.length};const de=this.focus==="confirm",yn=this.focus==="cancel";f(n,R,te,W,de?"black":"white",de?"green":"black"),f(n,R,X,N,yn?"black":"white",yn?"green":"black")}}class ki extends we{constructor(n,t,r,i,s,o,a,l){const c=P(i),h=[];c.amenities.trader&&h.push({label:"TRADER",action:o}),c.amenities.missionBoard&&h.push({label:"MISSION BOARD",action:a});const d=r.fuelCapacityL-r.fuelL,u=Math.floor(r.credits/fe),p=Math.min(d,u);let m=null;if(c.amenities.fuel&&p>0){const E=p*fe;m=h.length,h.push({label:`BUY FUEL  +${p}L  ${E}CR`,action:()=>{}})}const y=ln(c.description,36).slice(0,3),g=`DANGER: ${c.dangerLevel.toUpperCase()}`,v=[...y,g],A=c.locationType==="surface"||c.locationType==="asteroid"?"TAKE OFF":"UNDOCK";super("HUB",h,[{id:"undock",label:A}],n,t,r,v),this.onShip=l,this.onRefuel=s,this.fuelItemIdx=m}activateCurrent(){const n=this.items;if(n.length===0||this.cursorIdx===-1)return;const t=n[this.cursorIdx];if(!t.disabled){if(this.fuelItemIdx!==null&&this.cursorIdx===this.fuelItemIdx){this.activated=!0;const r=this.player.fuelCapacityL-this.player.fuelL,i=Math.floor(this.player.credits/fe),s=Math.min(r,i);this.openModal(new rn({title:"BUY FUEL",field:{label:"Litres",initialValue:s,min:0,max:s},derivedRows:[{label:"Cost",compute:o=>`${o*fe} CR`}],confirmLabel:"BUY",onConfirm:o=>{this.closeModal(),o>0&&this.onRefuel(o*fe,o)},onCancel:()=>{this.closeModal(),this.activated=!1}}));return}this.activated=!0,t.action()}}handleNavAction(n){(n==="BACK"||n==="NAV_1")&&!this.activated&&(this.activated=!0,this.onShip())}handleNavTap(n){n==="undock"&&!this.activated&&(this.activated=!0,this.onShip())}}class xi extends we{constructor(n,t,r,i,s,o,a,l,c){var p;const d=((p=P(i).npcs.trader)==null?void 0:p.toUpperCase())??"TRADER",u=[{label:"BUY",items:[]},{label:"SELL",items:[]}];super(d,[],[{id:"undock",label:"UNDOCK"},{id:"hub",label:"HUB"}],n,t,r,[],u),this.traderStock=s,this.onBuy=o,this.onSell=a,this.onHub=l,this.onUndock=c,this.syncItems(),this.clampCursor()}buildBuyItems(){return this.traderStock.length===0?[{label:"NO STOCK AVAILABLE",disabled:!0,action:()=>{}}]:this.traderStock.flatMap(n=>{const t=ee(n.commodityId);if(!t)return[];const r=this.player.credits>=t.basePrice;return[{label:`${t.name} (x${n.qty})`,info:`${t.basePrice} CR`,disabled:!r,action:()=>{const i=Math.floor(this.player.credits/t.basePrice),s=Math.min(n.qty,i);this.openModal(new rn({title:t.name.toUpperCase(),field:{label:"Quantity",initialValue:s,min:0,max:s},derivedRows:[{label:"Total",compute:o=>`${o*t.basePrice} CR`}],confirmLabel:"BUY",onConfirm:o=>{o>0&&this.onBuy(n.commodityId,o),this.syncItems(),this.clampCursor(),this.closeModal()},onCancel:()=>this.closeModal()}))}}]})}buildSellItems(){const n=this.player.cargoHold;return n.length===0?[{label:"CARGO HOLD EMPTY",disabled:!0,action:()=>{}}]:[...n].flatMap(t=>{const r=ee(t.commodityId);return r?[{label:`${r.name} (x${t.qty})`,info:`${r.basePrice} CR`,action:()=>{this.openModal(new rn({title:r.name.toUpperCase(),field:{label:"Quantity",initialValue:t.qty,min:0,max:t.qty},derivedRows:[{label:"Total",compute:i=>`${i*r.basePrice} CR`}],confirmLabel:"SELL",onConfirm:i=>{i>0&&this.onSell(t.commodityId,i),this.syncItems(),this.clampCursor(),this.closeModal()},onCancel:()=>this.closeModal()}))}}]:[]})}syncItems(){this.tabs&&(this.tabs[0].items=this.buildBuyItems(),this.tabs[1].items=this.buildSellItems())}clampCursor(){var t;const n=this.items;(this.cursorIdx<0||this.cursorIdx>=n.length||(t=n[this.cursorIdx])!=null&&t.disabled)&&(this.cursorIdx=n.findIndex(r=>!r.disabled))}activateCurrent(){const n=this.items;if(n.length===0||this.cursorIdx<0||this.cursorIdx>=n.length)return;const t=n[this.cursorIdx];t.disabled||t.action()}handleNavAction(n){(n==="BACK"||n==="NAV_2")&&!this.activated?(this.activated=!0,this.onHub()):n==="NAV_1"&&!this.activated&&(this.activated=!0,this.onUndock())}handleNavTap(n){n==="hub"&&!this.activated?(this.activated=!0,this.onHub()):n==="undock"&&!this.activated&&(this.activated=!0,this.onUndock())}render(n){this.syncItems(),super.render(n);const t=n.length,r=`HOLD: ${this.player.cargoWeightKg}/${this.player.cargoCapacity}KG`,i=pt(t,!0)-2;f(n,i,2,r,"bright-black","black")}}const Ci={delivery:"[D] ",supply:"[S] "};class Ti extends we{constructor(n,t,r,i,s,o,a,l){P(i);const c=s();let h;c.length===0?h=[{label:"NO MISSIONS AVAILABLE",disabled:!0,action:()=>{}}]:h=c.map(d=>({label:d.title,icon:Ci[d.type],iconFg:"bright-yellow",info:`${d.reward} CR`,infoFg:"bright-green",action:()=>o(d)})),super("MISSION BOARD",h,[{id:"undock",label:"UNDOCK"},{id:"hub",label:"HUB"}],n,t,r),this.onHub=a,this.onUndock=l}handleNavAction(n){(n==="BACK"||n==="NAV_2")&&!this.activated?(this.activated=!0,this.onHub()):n==="NAV_1"&&!this.activated&&(this.activated=!0,this.onUndock())}handleNavTap(n){n==="hub"&&!this.activated?(this.activated=!0,this.onHub()):n==="undock"&&!this.activated&&(this.activated=!0,this.onUndock())}}function Ai(e,n){return e.type==="delivery"?e.pickupComplete?n.destinationId===e.deliveryDestinationId?"ready-to-deliver":"in-transit":"pending-pickup":e.requirements.every(r=>{const i=n.cargoHold.find(s=>s.commodityId===r.commodityId);return i!==void 0&&i.qty>=r.qty})?"ready-to-deliver":"needs-supplies"}function Si(e,n){return n.type==="delivery"&&e.cargoCapacity-e.cargoWeightKg<n.itemWeightKg?{ok:!1,reason:"Insufficient cargo space"}:{ok:!0}}class Ii{constructor(n){const t=ut(n.shipId);if(!t)throw new Error(`Unknown ship: ${n.shipId}`);this.shipId=n.shipId,this.driveId=n.driveId,this.fuelCapacityL=t.fuelCapacityL,this.cargoCapacity=t.cargoCapacityKg,this._fuelL=t.fuelCapacityL,this._credits=n.credits,this._systemId=n.systemId,this._destinationId=n.destinationId,this._cargoHold=[],this._activeMissions=[],this._missionItems=[]}get fuelL(){return this._fuelL}addFuel(n){this._fuelL=Math.min(this.fuelCapacityL,this._fuelL+n)}consumeFuel(n){this._fuelL=Math.max(0,this._fuelL-n)}get credits(){return this._credits}addCredits(n){this._credits+=n}spendCredits(n){this._credits-=n}get cargoHold(){return this._cargoHold}addCargo(n,t){const r=this._cargoHold.find(i=>i.commodityId===n);r?r.qty+=t:this._cargoHold.push({commodityId:n,qty:t})}removeCargo(n,t){const r=this._cargoHold.findIndex(i=>i.commodityId===n);r<0||(t!==void 0&&t<this._cargoHold[r].qty?this._cargoHold[r].qty-=t:this._cargoHold.splice(r,1))}get missionItemsWeightKg(){return this._missionItems.reduce((n,t)=>n+t.weightKg,0)}get cargoWeightKg(){return yi(this._cargoHold)+this.missionItemsWeightKg}get systemId(){return this._systemId}get destinationId(){return this._destinationId}dock(n){this._destinationId=n}undock(){this._destinationId=null}jumpTo(n){this._systemId=n,this._destinationId=null}get activeMissions(){return this._activeMissions}get missionItems(){return this._missionItems}acceptMission(n,t){const r={...n,acceptedAt:Date.now(),pickupComplete:!1};this._activeMissions.push(r),n.type==="delivery"&&t&&(this._missionItems.push({missionId:n.id,itemName:n.itemName,weightKg:n.itemWeightKg}),r.pickupComplete=!0)}collectMissionItem(n){const t=this._activeMissions.find(r=>r.id===n);!t||t.type!=="delivery"||(t.pickupComplete=!0,this._missionItems.push({missionId:t.id,itemName:t.itemName,weightKg:t.itemWeightKg}))}completeMission(n){this._activeMissions=this._activeMissions.filter(t=>t.id!==n),this._missionItems=this._missionItems.filter(t=>t.missionId!==n)}cancelMission(n){this._activeMissions=this._activeMissions.filter(t=>t.id!==n),this._missionItems=this._missionItems.filter(t=>t.missionId!==n)}getMissionsForPickup(n){return this._activeMissions.filter(t=>t.type==="delivery"&&t.pickupDestinationId===n&&!t.pickupComplete)}getMissionsForDelivery(n){return this._activeMissions.filter(t=>t.deliveryDestinationId===n&&Ai(t,this)==="ready-to-deliver")}}const Ei={delivery:"[D]",supply:"[S]"},Mi=16;class Li extends we{constructor(n,t,r,i,s,o,a,l){const c=Si(r,i),h=i.type==="delivery"&&i.pickupDestinationId===i.issuingDestinationId,d=c.ok?{label:"ACCEPT MISSION",action:()=>s(h)}:{label:"ACCEPT MISSION",disabled:!0,details:c.reason?[c.reason]:[],action:()=>{}},u={label:"BACK",action:()=>o()},p=Array.from({length:Mi},()=>"");super("MISSION BOARD",[d,u],[{id:"undock",label:"UNDOCK"},{id:"hub",label:"HUB"}],n,t,r,p),this.spec=i,this.onBack=o,this.onHub=a,this.onUndock=l}destColor(n){if(n===this.player.destinationId)return"bright-green";const t=P(n);return t&&t.system===this.player.systemId?"bright-yellow":"white"}writeDestRow(n,t,r,i,s,o){f(n,t,2,r,"white","black"),f(n,t,2+r.length,i.slice(0,o-r.length),this.destColor(s),"black")}handleNavAction(n){this.activated||(n==="NAV_1"?(this.activated=!0,this.onUndock()):n==="NAV_2"?(this.activated=!0,this.onHub()):n==="BACK"&&(this.activated=!0,this.onBack()))}handleNavTap(n){this.activated||(n==="undock"?(this.activated=!0,this.onUndock()):n==="hub"&&(this.activated=!0,this.onHub()))}render(n){super.render(n),this.renderDetail(n)}renderDetail(n){const r=n.length>0?n[0].length:40,i=r-4,s=this.itemStartRow-1;let o=C+3;const a=(u,p,m)=>{u<=s&&f(n,u,2,p.slice(0,i),m,"black")};a(o,`${Ei[this.spec.type]} ${this.spec.title}`,"bright-yellow"),o++;const l=U(),c=this.spec.giverFactionId?l.factions.find(u=>u.id===this.spec.giverFactionId):null,h=c?` [${c.name}]`:"";if(a(o,`    ${this.spec.giverName}${h}`,"bright-black"),o++,o++,this.spec.type==="delivery"){const u=P(this.spec.pickupDestinationId),p=P(this.spec.deliveryDestinationId);if(o<=s&&this.writeDestRow(n,o,"Pickup:  ",(u==null?void 0:u.name)??this.spec.pickupDestinationId,this.spec.pickupDestinationId,i),o++,o<=s&&this.writeDestRow(n,o,"Deliver: ",(p==null?void 0:p.name)??this.spec.deliveryDestinationId,this.spec.deliveryDestinationId,i),o++,o<=s){const m=this.player.cargoCapacity-this.player.cargoWeightKg,y=this.spec.itemWeightKg,g=m>=y,v=`Weight:  ${y} kg  (Free: ${m} kg)`;f(n,o,2,v,"white","black");const A=g?"bright-green":"red",E=2+v.length+1;E<r&&f(n,o,E,g?"✓":"✗",A,"black")}o++}else{const u=P(this.spec.deliveryDestinationId);o<=s&&this.writeDestRow(n,o,"Deliver to: ",(u==null?void 0:u.name)??this.spec.deliveryDestinationId,this.spec.deliveryDestinationId,i),o++;for(const p of this.spec.requirements){if(o>s)break;const m=ee(p.commodityId);a(o,`  ${p.qty}x ${(m==null?void 0:m.name)??p.commodityId}`,"white"),o++}}o++;const d=ln(this.spec.description,i);for(const u of d){if(o>s)break;a(o,u,"white"),o++}o++,!(o>s)&&a(o,`REWARD: ${this.spec.reward} CR`,"bright-green")}}const Ri=[18,10,5],Oi=[".","*","+"],kn=[4e3,2e3,800],Ni=[9e3,5e3,2500],Fi=[null,"bright-black","white"],Di=["bright-black","white","bright-white"],Pi=["white","bright-white","bright-cyan"],Ie=3,xn=25,Ee=2,Cn=37,Tn=2*Math.PI;function Ui(e){let n=e>>>0;return()=>(n=Math.imul(n,1664525)+1013904223>>>0,n/4294967296)}class Bi{constructor(n=42){this.boundsSet=!1,this.rand=Ui(n),this.stars=[];for(let t=0;t<3;t++)for(let r=0;r<Ri[t];r++){const i=Ee+Math.floor(this.rand()*(Cn-Ee+1)),s=Ie+Math.floor(this.rand()*(xn-Ie+1)),o=this.rand()*Tn,a=kn[t]+this.rand()*(Ni[t]-kn[t]);this.stars.push({col:i,row:s,layer:t,twinklePhase:o,twinklePeriod:a})}}update(n){for(const t of this.stars)t.twinklePhase+=Tn/t.twinklePeriod*n}render(n,t,r,i,s){if(!this.boundsSet){this.boundsSet=!0;const o=xn-Ie,a=Cn-Ee;{const l=(r-t)/o,c=(s-i)/a;for(const h of this.stars)h.row=Math.round(t+(h.row-Ie)*l),h.col=Math.round(i+(h.col-Ee)*c)}}for(const o of this.stars){const{row:a,col:l,layer:c}=o;if(a<t||a>r||l<i||l>s)continue;const h=Math.sin(o.twinklePhase);let d;h>=.5?d=Pi[c]:h>=-.5?d=Di[c]:d=Fi[c],d!==null&&(n[a][l]={char:Oi[c],fg:d,bg:"black"})}}getStars(){return this.stars}}const Hi=6,qe=6,$i=23,Gi=23,Ve=10,Wi=0,ji=4,Yi=18,Ki=21,qi=35,Vi=39,An=12,zi=11,J=13,me=27,ze=28,Xi=12,Xe=40,Sn=200,Je=["> SYSTEM STATUS: ALL CLEAR","> DRIVE EFFICIENCY: 97%","> BEACON SIGNAL DETECTED ON 14.7 MHz","> TRADE ROUTE UPDATE: ELYSIUM CORRIDOR STABLE","> WARNING: DEBRIS FIELD DELTA-9 ACTIVE","> COMMS RELAY SIGNAL NOMINAL","> FUEL RESERVES OPTIMAL","> SECTOR SCAN COMPLETE — NO HOSTILES","> GRAVITATIONAL ANOMALY LOGGED AT BEARING 227","> TRANSPONDER HANDSHAKE: ACCEPTED"],Ji="#",In=["green","cyan","white","yellow"],En=["*",".","+","x"];function Mn(e){let n=e>>>0;return()=>(n=Math.imul(n,1664525)+1013904223>>>0,n/4294967296)}function Qi(e,n,t){return{col:n,row:t,char:Ji,color:In[Math.floor(e()*In.length)],phase:e()*2e4,period:1e4+e()*1e4,active:e()>.2}}function ge(e,n,t,r){const i=[];for(const s of r)for(let o=n;o<=t;o++)i.push(Qi(e,o,s));return i}class Zi{constructor(n,t,r,i,s,o){this.cursorIdx=0,this.activated=!1,this.h=30,this.blinkPhase=0,this.msgIdx=0,this.tickerScroll=0,this.tickerAccum=0,this.tickerPause=0,this.player=r,this.chrome=new ve(t,r),this.starfield=new Bi(42),this.inSpace=r.destinationId===null;const a=Mn(99);this.gaugeBtns=[...ge(a,Wi,ji,[3,4]),...ge(a,Yi,Ki,[3,4]),...ge(a,qi,Vi,[3,4])],this.leftBtns=ge(a,0,zi,[0,1,2,3]),this.rightBtns=ge(a,ze,39,[0,1,2,3]);const l=Mn(77),c=3+Math.floor(l()*4);this.radarContacts=Array.from({length:c},()=>({x:l()*(me-J-1),y:l()*4,vx:(l()-.5)*2,vy:(l()-.5)*1.5,char:En[Math.floor(l()*En.length)]}));const h=()=>this.inSpace?1:2;n.onAction(d=>{this.activated||(d==="CARGO"?(this.activated=!0,o()):d==="UP"?this.cursorIdx=(this.cursorIdx-1+h())%h():d==="DOWN"?this.cursorIdx=(this.cursorIdx+1)%h():d==="SELECT"&&(this.cursorIdx===0?(this.activated=!0,i()):this.inSpace||(this.activated=!0,s())))}),n.onTap&&n.onTap((d,u)=>{if(this.activated)return;const p=this.h;(u===3||u===4)&&d>=qe&&d<qe+1+Ve?(this.activated=!0,o()):u===p-3&&d<An?(this.activated=!0,i()):u===p-3&&d>=ze&&!this.inSpace&&(this.activated=!0,s())})}update(n){this.starfield.update(n),this.blinkPhase=(this.blinkPhase+n)%1e3;for(const i of[...this.gaugeBtns,...this.leftBtns,...this.rightBtns])i.phase+=n,i.phase>=i.period&&(i.phase-=i.period,i.active=!i.active);const t=me-J,r=5;for(const i of this.radarContacts)i.x+=i.vx*n/1e3,i.y+=i.vy*n/1e3,i.x<0&&(i.x=-i.x,i.vx=-i.vx),i.x>t-1&&(i.x=2*(t-1)-i.x,i.vx=-i.vx),i.y<0&&(i.y=-i.y,i.vy=-i.vy),i.y>r-1&&(i.y=2*(r-1)-i.y,i.vy=-i.vy);if(this.tickerPause>0)this.tickerPause=Math.max(0,this.tickerPause-n);else for(this.tickerAccum+=n;this.tickerAccum>=Sn;){this.tickerAccum-=Sn,this.tickerScroll++;const i=Je[this.msgIdx];if(this.tickerScroll>=i.length+Xe-1){this.tickerScroll=0,this.msgIdx=(this.msgIdx+1)%Je.length,this.tickerPause=500,this.tickerAccum=0;break}}}render(n){const t=n.length,r=t>0?n[0].length:0;this.h=t;for(let c=0;c<t;c++)for(let h=0;h<r;h++)n[c][h]={char:" ",fg:"black",bg:"black"};this.chrome.render(n,{showHeader:!0,showFooter:!0,navOptions:[]});const i=5,s=t-8,o=t-7,a=t-3,l=t-2;this.renderGaugeStrip(n),this.starfield.render(n,i,s,0,39);for(let c=0;c<r;c++)n[i][c]={char:"-",fg:"white",bg:"black"},n[s][c]={char:"-",fg:"white",bg:"black"};this.renderHUD(n,i+1),this.renderCrosshair(n,i+1,s-1),this.renderBottomPanels(n,o,a),this.renderTicker(n,l)}renderGaugeStrip(n){for(const s of this.gaugeBtns){const o=s.active?s.color:"bright-black";n[s.row][s.col]={char:s.char,fg:o,bg:"black"}}const t=this.player.fuelL/this.player.fuelCapacityL,r=this.player.cargoWeightKg/this.player.cargoCapacity,i=this.blinkPhase<500;this.renderGauge(n,3,Hi,"F",t,"yellow",i),this.renderGauge(n,4,qe,"C",r,"blue",i),this.renderGauge(n,3,$i,"S",1,"cyan",i),this.renderGauge(n,4,Gi,"H",1,"green",i)}renderGauge(n,t,r,i,s,o,a){n[t][r]={char:i,fg:o,bg:"black"};const l=Math.round(Math.min(1,Math.max(0,s))*Ve),c=s<=.2;for(let h=0;h<Ve;h++){const d=r+1+h;if(h<l){const u=c&&!a?"bright-black":o;n[t][d]={char:" ",fg:"black",bg:u}}else n[t][d]={char:" ",fg:"black",bg:"bright-black"}}}renderHUD(n,t){f(n,t,1,"VEL:----","bright-black","black"),f(n,t,16,"ATT:---°","bright-black","black"),f(n,t,30,"ROT:--°","bright-black","black")}renderCrosshair(n,t,r){const i=Math.floor((t+r)/2),s=20;n[i][s]={char:"+",fg:"bright-green",bg:"black"};const o=[[i-3,s-5],[i-3,s+5],[i+3,s-5],[i+3,s+5]];for(const[a,l]of o)a>=t&&a<=r&&l>=0&&l<40&&(n[a][l]={char:"+",fg:"bright-green",bg:"black"})}renderBottomPanels(n,t,r){for(let c=t;c<=r;c++)for(let h=J;h<me;h++)n[c][h]={char:" ",fg:"black",bg:"bright-black"};this.renderRadar(n,t,r-t+1);for(const c of this.leftBtns){const h=t+c.row;if(h<r){const d=c.active?c.color:"bright-black";n[h][c.col]={char:c.char,fg:d,bg:"black"}}}for(const c of this.rightBtns){const h=t+c.row;if(h<r){const d=c.active?c.color:"bright-black";n[h][c.col]={char:c.char,fg:d,bg:"black"}}}const i=this.cursorIdx===0?"bright-yellow":"yellow";f(n,r,0,this.centerPad("TRAVEL",An),"black",i);let s,o;this.inSpace?(s="bright-black",o="bright-black"):(s=this.cursorIdx===1?"bright-cyan":"cyan",o="black"),f(n,r,ze,this.centerPad("DOCK",Xi),o,s);const a=me-J,l="<)) "+"-".repeat(a-4);f(n,r,J,l,"white","bright-black")}renderRadar(n,t,r){for(const i of this.radarContacts){const s=Math.min(me-J-1,Math.max(0,Math.floor(i.x))),o=Math.min(r-1,Math.max(0,Math.floor(i.y)));n[t+o][J+s]={char:i.char,fg:"white",bg:"bright-black"}}}renderTicker(n,t){const r=Je[this.msgIdx];for(let i=0;i<Xe;i++){const s=this.tickerScroll-Xe+1+i,o=s>=0&&s<r.length?r[s]:" ";n[t][i]={char:o,fg:"white",bg:"black"}}}centerPad(n,t){if(n.length>=t)return n.slice(0,t);const r=t-n.length,i=Math.floor(r/2);return" ".repeat(i)+n+" ".repeat(r-i)}}const Qe="CARGO HOLD";class er{constructor(n,t,r,i){this.activated=!1,this.player=r,n.onAction(s=>{this.activated||(s==="BACK"||s==="CARGO")&&(this.activated=!0,i())})}update(n){}render(n){const t=n.length,r=t>0?n[0].length:0;for(let h=0;h<t;h++)for(let d=0;d<r;d++)n[h][d]={char:" ",fg:"black",bg:"black"};k(n,1,Qe,"bright-white","black");const i=Math.max(0,Math.floor((r-Qe.length)/2));f(n,2,i,"'".repeat(Qe.length),"bright-black","black");const s=this.player.cargoHold,o=this.player.cargoCapacity,a=this.player.cargoWeightKg;if(s.length===0)k(n,Math.floor(t/2),"CARGO HOLD EMPTY","bright-black","black");else{let h=4;for(const u of s){if(h>=t-3)break;const p=ee(u.commodityId);if(!p)continue;const m=u.qty*p.weightKg,y=`  x${u.qty}  ${p.basePrice}CR  ${m}KG`,g=Math.max(6,r-4-y.length),v=p.name,E=`${v.length>g?v.slice(0,g):v}${y}`;f(n,h,2,E,"white","black"),h++}const d=t-4;d>3&&f(n,d,2,"-".repeat(r-4),"bright-black","black")}const l=t-3,c=`TOTAL: ${a}/${o}KG`;f(n,l,2,c,"bright-black","black"),f(n,t-1,2,"[ESC] BACK","bright-black","black")}}class Ln extends we{constructor(n,t,r,i,s,o,a,l){const c=De(r.systemId),h=dt(r.driveId),d=[...c.destinations.map(y=>({label:P(y).name.toUpperCase(),disabled:y===r.destinationId,action:()=>i(y)})),{label:"FLY INTO SPACE",disabled:r.destinationId===null,action:o}],p=[...cn(r.systemId).map(y=>{const g=y.from===r.systemId?y.to:y.from,v=De(g),A=y.stability.toUpperCase(),E=Math.ceil(ft*y.distance*h.fuelEfficiency);return{label:`${v.name.toUpperCase()}  ${y.distance}LY  [${A}]`.slice(0,36),disabled:E>r.fuelL,action:()=>s(g)}}),{label:"GALAXY MAP...",action:l}],m=[{label:"DESTINATIONS",items:d},{label:"JUMPS",items:p}];super("TRAVEL",[],[{id:"ship",label:"SHIP"}],n,t,r,[],m),this.onShip=a}handleNavAction(n){(n==="BACK"||n==="NAV_1")&&!this.activated&&(this.activated=!0,this.onShip())}handleNavTap(n){n==="ship"&&!this.activated&&(this.activated=!0,this.onShip())}}function Rn(e,n){if(e===n)return[e];const t=[[e]],r=new Set([e]);for(;t.length>0;){const i=t.shift(),s=i[i.length-1];for(const o of cn(s)){const a=o.from===s?o.to:o.from;if(a===n)return[...i,a];r.has(a)||(r.add(a),t.push([...i,a]))}}return null}const On="GALAXY MAP",Nn=C+3,Fn=C+5,K=C+9,Dn=C+13,nr=C+14,Me=C+15,Ze=C+20,en=C+21,tr=2,ir=10,rr=12,Le=13,Re=12,or=25,sr=26,ar=10,Pn=18;function lr(e,n){return e.length>=n?e.slice(0,n):e+" ".repeat(n-e.length)}function ye(e,n){return"["+lr(e.toUpperCase(),n-2)+"]"}class cr{constructor(n,t,r,i){this.activeTab="map",this.mapCursorIdx=0,this.routeDestIdx=0,this.searchText="",this.activated=!1,this.chrome=new ve(t,r),this.player=r,this.onBack=i,this.publicSystems=gi().sort((s,o)=>s.distanceFromSol-o.distanceFromSol),this.otherSystems=this.publicSystems.filter(s=>s.id!==r.systemId),this.mapBrowsingSystemId=r.systemId,n.onCharInput&&n.onCharInput(s=>{if(this.activated||this.activeTab!=="map")return;const o=s.charCodeAt(0);s==="\b"||s===""?this.searchText=this.searchText.slice(0,-1):o>=32&&o<127&&(this.searchText+=s.toUpperCase(),this.mapCursorIdx=0)}),n.onAction(s=>{if(!this.activated){if(s==="BACK"){if(this.searchText.length>0){this.searchText="";return}this.activated=!0,this.onBack();return}if(s==="LEFT"){this.activeTab="map",this.searchText="";return}if(s==="RIGHT"){this.activeTab="route",this.searchText="";return}this.activeTab==="map"?this.handleMapAction(s):this.handleRouteAction(s)}}),n.onTap&&n.onTap((s,o)=>{if(this.activated)return;if(this.chrome.hitTestNav(s,o)==="back"){this.searchText="",this.activated=!0,this.onBack();return}if(o===Nn){s>=3&&s<=7?(this.activeTab="map",this.searchText=""):s>=9&&s<=16&&(this.activeTab="route",this.searchText="");return}if(this.activeTab==="map"&&o>=Me&&o<Ze){const l=this.getMapNeighbors(),c=o-Me;c>=0&&c<l.length&&(this.mapBrowsingSystemId=l[c].id,this.mapCursorIdx=0,this.searchText="")}if(this.activeTab==="route"&&o>=C+8&&o<=C+14){const l=o-(C+8);l>=0&&l<this.otherSystems.length&&(this.routeDestIdx=l)}})}getMapNeighbors(){const t=cn(this.mapBrowsingSystemId).map(r=>{const i=r.from===this.mapBrowsingSystemId?r.to:r.from,s=this.publicSystems.find(a=>a.id===i),o=r.distance;return s?{sys:s,dist:o}:null}).filter(r=>r!==null).sort((r,i)=>r.dist-i.dist).map(r=>r.sys);return this.searchText.length===0?t:t.filter(r=>r.name.toUpperCase().includes(this.searchText))}handleMapAction(n){const t=this.getMapNeighbors(),r=Math.min(this.mapCursorIdx,Math.max(0,t.length-1));if(n==="UP")this.mapCursorIdx=Math.max(0,r-1);else if(n==="DOWN")this.mapCursorIdx=Math.min(t.length-1,r+1);else if(n==="SELECT"){const i=t[r];if(!i)return;this.mapBrowsingSystemId=i.id,this.mapCursorIdx=0,this.searchText=""}}handleRouteAction(n){n==="UP"?this.routeDestIdx=Math.max(0,this.routeDestIdx-1):n==="DOWN"&&(this.routeDestIdx=Math.min(this.otherSystems.length-1,this.routeDestIdx+1))}computeRoute(){const n=this.otherSystems[this.routeDestIdx];return n?Rn(this.player.systemId,n.id):null}update(n){}render(n){const t=n.length,r=t>0?n[0].length:0;for(let s=0;s<t;s++)for(let o=0;o<r;o++)n[s][o]={char:" ",fg:"black",bg:"black"};const i=[{id:"back",label:"BACK"}];this.chrome.render(n,{showHeader:!0,showFooter:!0,navOptions:i}),f(n,C,2,On,"bright-white","black"),f(n,C+1,2,"'".repeat(On.length),"bright-black","black"),this.renderTabBar(n,r),this.activeTab==="map"?this.renderMapTab(n,r):this.renderRouteTab(n,r)}renderTabBar(n,t){const r=Nn;let i=2;n[r][i++]={char:"|",fg:"bright-black",bg:"black"};for(const[s,o]of[["MAP","map"],["ROUTE","route"]]){const a=this.activeTab===o,l=a?"black":"white",c=a?"green":"black";for(const h of` ${s} `)i<t&&(n[r][i]={char:h,fg:l,bg:c}),i++;i<t&&(n[r][i]={char:"|",fg:"bright-black",bg:"black"}),i++}}renderMapTab(n,t){const r=this.publicSystems.find(o=>o.id===this.mapBrowsingSystemId)??this.publicSystems[0];if(!r)return;this.renderChart(n,r),f(n,nr,0,"-".repeat(t),"bright-black","black");const i=this.getMapNeighbors(),s=Math.min(this.mapCursorIdx,Math.max(0,i.length-1));this.renderNeighborList(n,t,r,i,s),f(n,Ze,0,"-".repeat(t),"bright-black","black"),this.renderInfo(n,t,r,i[s]??null),this.searchText.length>0&&f(n,en+4,2,`/${this.searchText}_`,"bright-yellow","black")}renderChart(n,t){const r=this.getMapNeighbors(),i=t.id===this.player.systemId?"bright-yellow":"bright-cyan";if(f(n,K,Le,ye(t.name,Re),i,"black"),t.id===this.player.systemId){const o=Le+Re;n[K][o]={char:"*",fg:"bright-yellow",bg:"black"}}const s=["left","right","top","bottom"];for(let o=0;o<Math.min(r.length,4);o++){const a=r[o],l=s[o],c=a.id===this.player.systemId?"bright-yellow":"white";if(l==="left")f(n,K,tr,ye(a.name,ir),c,"black"),n[K][rr]={char:"-",fg:"bright-black",bg:"black"};else if(l==="right")f(n,K,sr,ye(a.name,ar),c,"black"),n[K][or]={char:"-",fg:"bright-black",bg:"black"};else if(l==="top"){f(n,Fn,Le,ye(a.name,Re),c,"black");for(let h=Fn+1;h<K;h++)n[h][Pn]={char:"|",fg:"bright-black",bg:"black"}}else{f(n,Dn,Le,ye(a.name,Re),c,"black");for(let h=K+1;h<Dn;h++)n[h][Pn]={char:"|",fg:"bright-black",bg:"black"}}}}renderNeighborList(n,t,r,i,s){for(let o=0;o<i.length&&o<Ze-Me;o++){const a=i[o],l=Me+o,c=o===s,d=a.id===this.player.systemId?"bright-yellow":c?"bright-cyan":"white",u=c?"> ":"  ",p=Oe(r.id,a.id),m=p?`${p.distance}LY  ${p.stability}`:"",y=t-4-m.length;f(n,l,2,u+a.name.toUpperCase().slice(0,y-2),d,"black"),m&&f(n,l,t-2-m.length,m,"bright-black","black")}}renderInfo(n,t,r,i){const o=r.id===this.player.systemId?"* Current location":r.name.toUpperCase();if(f(n,en,2,`Zone: ${r.zone}  Sec: ${r.security}  ${o}`.slice(0,t-4),"bright-black","black"),!i)return;const a=Oe(r.id,i.id);if(!a)return;const l=Rn(this.player.systemId,i.id),c=l?l.length===1?"(your location)":`${l.length-1} hop${l.length-1!==1?"s":""} from you`:"(unreachable)";f(n,en+1,2,`${i.name.toUpperCase()}  ${a.distance}LY  ${c}`.slice(0,t-4),"bright-black","black")}renderRouteTab(n,t){var y,g;const r=C+5,i=C+7,s=C+8,o=7,a=s+o,l=a+1,c=l+6,h=this.publicSystems.find(v=>v.id===this.player.systemId);f(n,r,2,"FROM:","bright-black","black"),f(n,r,8,(h==null?void 0:h.name.toUpperCase())??this.player.systemId.toUpperCase(),"bright-yellow","black"),f(n,i,2,"TO:","bright-black","black");const d=Math.max(0,Math.min(this.routeDestIdx,this.otherSystems.length-o));for(let v=0;v<o;v++){const A=d+v;if(A>=this.otherSystems.length)break;const E=this.otherSystems[A],w=A===this.routeDestIdx,b=w?"bright-cyan":"white",S=w?"> ":"  ";f(n,s+v,2,S+E.name.toUpperCase(),b,"black")}f(n,a,0,"-".repeat(t),"bright-black","black"),f(n,c,0,"-".repeat(t),"bright-black","black");const u=this.computeRoute();if(!this.otherSystems[this.routeDestIdx])return;if(!u){f(n,l,2,"No route found","bright-red","black");return}const m=u.length-1;f(n,l,2,`Route: ${m} hop${m!==1?"s":""}`,"bright-white","black");for(let v=0;v<m;v++){const A=Oe(u[v],u[v+1]);if(!A)continue;const E=l+1+v;if(E>=c)break;const w=(((y=this.publicSystems.find(S=>S.id===u[v]))==null?void 0:y.name)??u[v]).toUpperCase().slice(0,9),b=(((g=this.publicSystems.find(S=>S.id===u[v+1]))==null?void 0:g.name)??u[v+1]).toUpperCase().slice(0,9);f(n,E,4,`${w} -> ${b}  ${A.distance}LY  ${A.stability}`.slice(0,t-6),"white","black")}}}class z{constructor(n,t,r,i){this.elapsed=0,this.arrived=!1,this.player=n,this.chrome=new ve(t,n),this.duration=r,this.onComplete=i}update(n){this.arrived||(this.elapsed+=n,this.elapsed>=this.duration&&(this.arrived=!0,this.onComplete()))}render(n){const t=n.length,r=t>0?n[0].length:0;for(let i=0;i<t;i++)for(let s=0;s<r;s++)n[i][s]={char:" ",fg:"black",bg:"black"};this.chrome.render(n,this.getChromeConfig()),this.renderContent(n)}getChromeConfig(){return{showHeader:!0,showFooter:!0,navOptions:[]}}}const hr=["[. . .]","[: : :]","[* * *]"],Un=5e3;class dr extends z{constructor(n,t,r){super(n,t,Un,r)}getChromeConfig(){return{showHeader:!0,showFooter:!0,navOptions:[],systemLabel:"IN TRANSIT",destinationLabel:null}}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/500)%3,s=Math.ceil((Un-this.elapsed)/1e3),o=Math.max(1,Math.min(5,s)),a=De(this.player.systemId),l=a?a.name.toUpperCase():this.player.systemId.toUpperCase();k(n,r-3,"[ JUMP DRIVE ENGAGED ]","bright-cyan","black"),k(n,r-1,"DESTINATION:","bright-black","black"),k(n,r,l,"bright-white","black"),k(n,r+2,hr[i],"bright-black","black"),k(n,r+4,`ARRIVING IN ${o}S`,"bright-black","black")}}const Bn=2e3,ur=["[ —   ]","[  —  ]","[   — ]"];class Hn extends z{constructor(n,t,r,i){super(n,t,Bn,r),this.targetLabel=i}getChromeConfig(){return{showHeader:!0,showFooter:!0,navOptions:[],destinationLabel:"IN TRANSIT"}}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/300)%3,s=Math.ceil((Bn-this.elapsed)/1e3),o=Math.max(1,Math.min(2,s)),a=this.targetLabel!==void 0?this.targetLabel.toUpperCase():(()=>{const l=this.player.destinationId?P(this.player.destinationId):null;return l?l.name.toUpperCase():"UNKNOWN"})();k(n,r-3,"[ THRUSTERS ENGAGED ]","bright-yellow","black"),k(n,r-1,"HEADING TO:","bright-black","black"),k(n,r,a,"bright-white","black"),k(n,r+2,ur[i],"bright-black","black"),k(n,r+4,`ARRIVING IN ${o}S`,"bright-black","black")}}const $n=2500,pr=["v","vv","vvv"];class fr extends z{constructor(n,t,r){super(n,t,$n,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/400)%3,s=Math.ceil(($n-this.elapsed)/1e3),o=Math.max(1,Math.min(3,s));k(n,r-3,"[ LANDING SEQUENCE ]","bright-green","black"),k(n,r+2,pr[i],"bright-black","black"),k(n,r+4,`TOUCHDOWN IN ${o}S`,"bright-black","black")}}const Gn=2500,mr=[">",">>",">>>"];class gr extends z{constructor(n,t,r){super(n,t,Gn,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/400)%3,s=Math.ceil((Gn-this.elapsed)/1e3),o=Math.max(1,Math.min(3,s));k(n,r-3,"[ APPROACH LOCKED ]","bright-yellow","black"),k(n,r+2,mr[i],"bright-black","black"),k(n,r+4,`CLAMPING IN ${o}S`,"bright-black","black")}}const Wn=1500,yr=["^","^^","^^^"];class br extends z{constructor(n,t,r){super(n,t,Wn,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/300)%3,s=Math.ceil((Wn-this.elapsed)/1e3),o=Math.max(1,Math.min(2,s));k(n,r-3,"[ LIFTOFF SEQUENCE ]","bright-green","black"),k(n,r+2,yr[i],"bright-black","black"),k(n,r+4,`CLEAR IN ${o}S`,"bright-black","black")}}const jn=1500,_r=["<","<<","<<<"];class vr extends z{constructor(n,t,r){super(n,t,jn,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/300)%3,s=Math.ceil((jn-this.elapsed)/1e3),o=Math.max(1,Math.min(2,s));k(n,r-3,"[ RELEASING CLAMPS ]","bright-yellow","black"),k(n,r+2,_r[i],"bright-black","black"),k(n,r+4,`DEPARTING IN ${o}S`,"bright-black","black")}}const Yn=1500,wr=["→","→→","→→→"];class kr extends z{constructor(n,t,r){super(n,t,Yn,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/300)%3,s=Math.ceil((Yn-this.elapsed)/1e3),o=Math.max(1,Math.min(2,s));k(n,r-3,"[ DOCKING SEQUENCE ]","bright-cyan","black"),k(n,r+2,wr[i],"bright-black","black"),k(n,r+4,`DOCKING IN ${o}S`,"bright-black","black")}}const Kn=1500,xr=["←","←←","←←←"];class Cr extends z{constructor(n,t,r){super(n,t,Kn,r)}renderContent(n){const t=n.length,r=Math.floor(t/2),i=Math.floor(this.elapsed/300)%3,s=Math.ceil((Kn-this.elapsed)/1e3),o=Math.max(1,Math.min(2,s));k(n,r-3,"[ DEPARTING BERTH ]","bright-cyan","black"),k(n,r+2,xr[i],"bright-black","black"),k(n,r+4,`CLEAR IN ${o}S`,"bright-black","black")}}function Tr(e){let n=e>>>0;return()=>(n=Math.imul(n,1664525)+1013904223>>>0,n/4294967296)}function Ar(e,n){return Math.floor(e()*n)}function ae(e,n){return n[Ar(e,n.length)]}function mt(e,n){const{special:t,firstNames:r,lastNames:i}=n.npcNames;if(e()<.3&&t.length>0)return{giverName:ae(e,t)};const s=r.length>0?ae(e,r):"Unknown",o=i.length>0?ae(e,i):"Agent";return{giverName:`${s} ${o}`}}function Sr(e,n){return n.destinations.filter(t=>t.id!==e.id)}function Ir(e){return e.commodities.filter(n=>n.legal)}function Er(e,n,t,r){const i=t.deliveryItems;if(i.length===0)return null;const s=Sr(n,t);if(s.length===0)return null;const o=ae(e,i),a=ae(e,s),l=mt(e,t),c=200,h=Math.floor(o.weightKg*1.5),d=c+h+Math.floor(e()*200);return{...l,id:r,type:"delivery",title:o.name,description:`A package needs transporting. Pick up the ${o.name} from ${n.name} and deliver it to ${a.name}. Handle with care.`,reward:d,issuingDestinationId:n.id,itemName:o.name,itemWeightKg:o.weightKg,pickupDestinationId:n.id,deliveryDestinationId:a.id}}function Mr(e,n,t,r){const i=Ir(t);if(i.length===0)return null;const s=n.goodsBias.map(m=>m.toLowerCase()),o=[];for(const m of i){const y=s.some(g=>m.category.includes(g)||m.id.includes(g)||g.includes(m.category));o.push(m),y&&o.push(m)}const a=1+Math.floor(e()*2),l=[],c=new Set;for(let m=0;m<a;m++){let y=0;for(;y<10;){const g=ae(e,o);if(!c.has(g.id)){c.add(g.id);const v=1+Math.floor(e()*4);l.push({commodityId:g.id,qty:v});break}y++}}if(l.length===0)return null;const h=l.reduce((m,y)=>{const g=t.commodities.find(v=>v.id===y.commodityId);return m+((g==null?void 0:g.basePrice)??100)*y.qty},0),d=Math.floor(h*.4)+Math.floor(e()*150),u=mt(e,t),p=l.map(m=>{const y=t.commodities.find(g=>g.id===m.commodityId);return`${m.qty}× ${(y==null?void 0:y.name)??m.commodityId}`}).join(", ");return{...u,id:r,type:"supply",title:n.name,description:`${n.name} needs supplies. Deliver ${p} to fulfil the contract.`,reward:d,issuingDestinationId:n.id,requirements:l,deliveryDestinationId:n.id}}function Lr(e,n,t){const r=Tr(t),i=3+Math.floor(r()*4),s=[];for(let o=0;o<i;o++){const a=`m-${(t>>>0).toString(16)}-${o}`,c=r()<.6?Er(r,e,n,a):Mr(r,e,n,a);c&&s.push(c)}return s}const Rr=100,Or=2*60*1e3,Nr=15*60*1e3;class Fr{constructor(n,t,r){this.traderStockCache=new Map,this.missionBoardCache=new Map,this.renderer=n,this.input=t,this.context=r;const i=fi(),s=ut(i.startingShip);this.player=new Ii({shipId:i.startingShip,driveId:s.defaultJumpDrive,credits:i.player.startingCredits,systemId:i.startingLocation.system,destinationId:i.startingLocation.destination}),this.currentScene=new vn(this.input,this.context,this.player,()=>this.goToStory())}tick(n){const t=Math.min(n,Rr),r=this.makeBuffer();this.currentScene.update(t),this.currentScene.render(r),this.renderer.drawBuffer(r)}makeBuffer(){const n=this.renderer.getWidth(),t=this.renderer.getHeight();return Array.from({length:t},()=>Array.from({length:n},()=>({char:" ",fg:"black",bg:"black"})))}getOrCreateTraderStock(n){const t=Date.now(),r=this.traderStockCache.get(n);if(r&&t-r.generatedAt<Or)return r.entries;const i=mi(),s=4+Math.floor(Math.random()*3),o=[...i];for(let l=o.length-1;l>0;l--){const c=Math.floor(Math.random()*(l+1));[o[l],o[c]]=[o[c],o[l]]}const a=o.slice(0,s).map(l=>({commodityId:l.id,qty:1+Math.floor(Math.random()*8)}));return this.traderStockCache.set(n,{entries:a,generatedAt:t}),a}getOrCreateMissionBoard(n){const t=Date.now(),r=this.missionBoardCache.get(n);if(r&&t-r.generatedAt<Nr)return r.specs;const i=P(n),s=U(),o=Math.floor(Math.random()*4294967295),a=Lr(i,s,o);return this.missionBoardCache.set(n,{specs:a,generatedAt:t}),a}onBuy(n,t,r){if(t<=0)return;const i=r.findIndex(c=>c.commodityId===n);if(i<0)return;const s=r[i];if(t>s.qty)return;const o=ee(n);if(!o)return;const a=t*o.basePrice;this.player.credits<a||this.player.cargoWeightKg+t*o.weightKg>this.player.cargoCapacity||(this.player.spendCredits(a),this.player.addCargo(n,t),s.qty-=t,s.qty<=0&&r.splice(i,1))}onSell(n,t,r){if(t<=0)return;const i=this.player.cargoHold.find(l=>l.commodityId===n);if(!i||i.qty<t)return;const s=ee(n);if(!s)return;const o=t*s.basePrice;this.player.addCredits(o),this.player.removeCargo(n,t);const a=r.find(l=>l.commodityId===n);a?a.qty+=t:r.push({commodityId:n,qty:t})}goToMainMenu(){this.currentScene=new vn(this.input,this.context,this.player,()=>this.goToStory())}goToStory(){this.currentScene=new vi(this.input,this.context,this.player,()=>this.goToStation())}goToStation(){this.currentScene=new ki(this.input,this.context,this.player,this.player.destinationId,(n,t)=>{this.player.spendCredits(n),this.player.addFuel(t),this.goToStation()},()=>this.goToTrader(),()=>this.goToMissionBoard(),()=>this.goToTakeOffOrUndock())}goToLandOrDock(){var t;const n=(t=P(this.player.destinationId))==null?void 0:t.locationType;n==="surface"?this.currentScene=new fr(this.player,this.context,()=>this.goToStation()):n==="asteroid"?this.currentScene=new gr(this.player,this.context,()=>this.goToStation()):this.currentScene=new kr(this.player,this.context,()=>this.goToStation())}goToTakeOffOrUndock(){var t;const n=(t=P(this.player.destinationId))==null?void 0:t.locationType;n==="surface"?this.currentScene=new br(this.player,this.context,()=>this.goToShip()):n==="asteroid"?this.currentScene=new vr(this.player,this.context,()=>this.goToShip()):this.currentScene=new Cr(this.player,this.context,()=>this.goToShip())}goToTrader(){const n=this.player.destinationId,t=this.getOrCreateTraderStock(n);this.currentScene=new xi(this.input,this.context,this.player,n,t,(r,i)=>this.onBuy(r,i,t),(r,i)=>this.onSell(r,i,t),()=>this.goToStation(),()=>this.goToShip())}goToMissionBoard(){const n=this.player.destinationId;this.currentScene=new Ti(this.input,this.context,this.player,n,()=>this.getOrCreateMissionBoard(n),t=>this.goToMissionDetail(t,n),()=>this.goToStation(),()=>this.goToShip())}goToMissionDetail(n,t){this.currentScene=new Li(this.input,this.context,this.player,n,r=>this.onMissionAccepted(n,r,t),()=>this.goToMissionBoard(),()=>this.goToStation(),()=>this.goToShip())}onMissionAccepted(n,t,r){const i=this.missionBoardCache.get(r);if(i){const s=i.specs.findIndex(o=>o.id===n.id);s>=0&&i.specs.splice(s,1)}this.player.acceptMission(n,t),this.goToMissionBoard()}goToShip(){this.currentScene=new Zi(this.input,this.context,this.player,()=>this.goToTravelMenu(),()=>this.goToLandOrDock(),()=>this.goToCargo())}goToCargo(){this.currentScene=new er(this.input,this.context,this.player,()=>this.goToShip())}goToTravelMenu(){this.currentScene=new Ln(this.input,this.context,this.player,n=>this.onDestinationSelected(n),n=>this.onJumpSelected(n),()=>this.goToFlyIntoSpace(),()=>this.goToShip(),()=>this.goToGalaxyMap())}goToArrival(){this.currentScene=new Ln(this.input,this.context,this.player,n=>this.onDestinationSelected(n),n=>this.onJumpSelected(n),()=>this.goToFlyIntoSpace(),()=>this.goToShip(),()=>this.goToGalaxyMap())}goToGalaxyMap(){this.currentScene=new cr(this.input,this.context,this.player,()=>this.goToTravelMenu())}goToFlyIntoSpace(){this.player.undock(),this.currentScene=new Hn(this.player,this.context,()=>this.goToShip(),"OPEN SPACE")}onDestinationSelected(n){this.player.dock(n),this.currentScene=new Hn(this.player,this.context,()=>this.goToShip())}onJumpSelected(n){const t=Oe(this.player.systemId,n),r=dt(this.player.driveId),i=Math.ceil(ft*t.distance*r.fuelEfficiency);this.player.consumeFuel(i),this.player.jumpTo(n),this.currentScene=new dr(this.player,this.context,()=>this.goToArrival())}}const Dr=`---
commodities:
...
Adventure here is fraught with inconsistent communications and unreliable star charts.`,fo=`---
id: game-settings
...
Long range engines for faster than light travel between systems.`,_o=`---
id: freighter
...
`;var T={},ke={},B={};function gt(e){return typeof e>"u"||e===null}function Fo(e){return typeof e=="object"&&e!==null}function Do(e){return Array.isArray(e)?e:gt(e)?[]:[e]}function Po(e,n){var t,r,i,s;if(n)for(s=Object.keys(n),t=0,r=s.length;t<r;t+=1)i=s[t],e[i]=n[i];return e}function Uo(e,n){var t="",r;for(r=0;r<n;r+=1)t+=e;return t}function Bo(e){return e===0&&Number.NEGATIVE_INFINITY===1/e}B.isNothing=gt;B.isObject=Fo;B.toArray=Do;B.repeat=Uo;B.isNegativeZero=Bo;B.extend=Po;function be(e,n){Error.call(this),this.name="YAMLException",this.reason=e,this.mark=n,this.message=(this.reason||"(unknown reason)")+(this.mark?" "+this.mark.toString():""),Error.captureStackTrace?Error.captureStackTrace(this,this.constructor):this.stack=new Error().stack||""}be.prototype=Object.create(Error.prototype);be.prototype.constructor=be;be.prototype.toString=function(n){var t=this.name+": ";return t+=this.reason||"(unknown reason)",!n&&this.mark&&(t+=" "+this.mark.toString()),t};var xe=be,qn=B;function hn(e,n,t,r,i){this.name=e,this.buffer=n,this.position=t,this.line=r,this.column=i}hn.prototype.getSnippet=function(n,t){var r,i,s,o,a;if(!this.buffer)return null;for(n=n||4,t=t||75,r="",i=this.position;i>0&&`\0\r
...
`+qn.repeat(" ",n+this.position-i+r.length)+"^"};hn.prototype.toString=function(n){var t,r="";return this.name&&(r+='in "'+this.name+'" '),r+="at line "+(this.line+1)+", column "+(this.column+1),n||(t=this.getSnippet(),t&&(r+=`:
`+t)),r};var Ho=hn,Vn=xe,$o=["kind","resolve","construct","instanceOf","predicate","represent","defaultStyle","styleAliases"],Go=["scalar","sequence","mapping"];function Wo(e){var n={};return e!==null&&Object.keys(e).forEach(function(t){e[t].forEach(function(r){n[String(r)]=t})}),n}function jo(e,n){if(n=n||{},Object.keys(n).forEach(function(t){if($o.indexOf(t)===-1)throw new Vn('Unknown option "'+t+'" is met in definition of "'+e+'" YAML type.')}),this.tag=e,this.kind=n.kind||null,this.resolve=n.resolve||function(){return!0},this.construct=n.construct||function(t){return t},this.instanceOf=n.instanceOf||null,this.predicate=n.predicate||null,this.represent=n.represent||null,this.defaultStyle=n.defaultStyle||null,this.styleAliases=Wo(n.styleAliases||null),Go.indexOf(this.kind)===-1)throw new Vn('Unknown kind "'+this.kind+'" is specified for "'+e+'" YAML type.')}var L=jo,zn=B,Ne=xe,Yo=L;function on(e,n,t){var r=[];return e.include.forEach(function(i){t=on(i,n,t)}),e[n].forEach(function(i){t.forEach(function(s,o){s.tag===i.tag&&s.kind===i.kind&&r.push(o)}),t.push(i)}),t.filter(function(i,s){return r.indexOf(s)===-1})}function Ko(){var e={scalar:{},sequence:{},mapping:{},fallback:{}},n,t;function r(i){e[i.kind][i.tag]=e.fallback[i.tag]=i}for(n=0,t=arguments.length;n<t;n+=1)arguments[n].forEach(r);return e}function re(e){this.include=e.include||[],this.implicit=e.implicit||[],this.explicit=e.explicit||[],this.implicit.forEach(function(n){if(n.loadKind&&n.loadKind!=="scalar")throw new Ne("There is a non-scalar type in the implicit list of a schema. Implicit resolving of such types is not supported.")}),this.compiledImplicit=on(this,"implicit",[]),this.compiledExplicit=on(this,"explicit",[]),this.compiledTypeMap=Ko(this.compiledImplicit,this.compiledExplicit)}re.DEFAULT=null;re.create=function(){var n,t;switch(arguments.length){case 1:n=re.DEFAULT,t=arguments[0];break;case 2:n=arguments[0],t=arguments[1];break;default:throw new Ne("Wrong number of arguments for Schema.create function")}if(n=zn.toArray(n),t=zn.toArray(t),!n.every(function(r){return r instanceof re}))throw new Ne("Specified list of super schemas (or a single Schema object) contains a non-Schema object.");if(!t.every(function(r){return r instanceof Yo}))throw new Ne("Specified list of YAML types (or a single Type object) contains a non-Type object.");return new re({include:n,explicit:t})};var he=re,qo=L,Vo=new qo("tag:yaml.org,2002:str",{kind:"scalar",construct:function(e){return e!==null?e:""}}),zo=L,Xo=new zo("tag:yaml.org,2002:seq",{kind:"sequence",construct:function(e){return e!==null?e:[]}}),Jo=L,Qo=new Jo("tag:yaml.org,2002:map",{kind:"mapping",construct:function(e){return e!==null?e:{}}}),Zo=he,dn=new Zo({explicit:[Vo,Xo,Qo]}),es=L;function ns(e){if(e===null)return!0;var n=e.length;return n===1&&e==="~"||n===4&&(e==="null"||e==="Null"||e==="NULL")}function ts(){return null}function is(e){return e===null}var rs=new es("tag:yaml.org,2002:null",{kind:"scalar",resolve:ns,construct:ts,predicate:is,represent:{canonical:function(){return"~"},lowercase:function(){return"null"},uppercase:function(){return"NULL"},camelcase:function(){return"Null"}},defaultStyle:"lowercase"}),os=L;function ss(e){if(e===null)return!1;var n=e.length;return n===4&&(e==="true"||e==="True"||e==="TRUE")||n===5&&(e==="false"||e==="False"||e==="FALSE")}function as(e){return e==="true"||e==="True"||e==="TRUE"}function ls(e){return Object.prototype.toString.call(e)==="[object Boolean]"}var cs=new os("tag:yaml.org,2002:bool",{kind:"scalar",resolve:ss,construct:as,predicate:ls,represent:{lowercase:function(e){return e?"true":"false"},uppercase:function(e){return e?"TRUE":"FALSE"},camelcase:function(e){return e?"True":"False"}},defaultStyle:"lowercase"}),hs=B,ds=L;function us(e){return 48<=e&&e<=57||65<=e&&e<=70||97<=e&&e<=102}function ps(e){return 48<=e&&e<=55}function fs(e){return 48<=e&&e<=57}function ms(e){if(e===null)return!1;var n=e.length,t=0,r=!1,i;if(!n)return!1;if(i=e[t],(i==="-"||i==="+")&&(i=e[++t]),i==="0"){if(t+1===n)return!0;if(i=e[++t],i==="b"){for(t++;t<n;t++)if(i=e[t],i!=="_"){if(i!=="0"&&i!=="1")return!1;r=!0}return r&&i!=="_"}if(i==="x"){for(t++;t<n;t++)if(i=e[t],i!=="_"){if(!us(e.charCodeAt(t)))return!1;r=!0}return r&&i!=="_"}for(;t<n;t++)if(i=e[t],i!=="_"){if(!ps(e.charCodeAt(t)))return!1;r=!0}return r&&i!=="_"}if(i==="_")return!1;for(;t<n;t++)if(i=e[t],i!=="_"){if(i===":")break;if(!fs(e.charCodeAt(t)))return!1;r=!0}return!r||i==="_"?!1:i!==":"?!0:/^(:[0-5]?[0-9])+$/.test(e.slice(t))}function gs(e){var n=e,t=1,r,i,s=[];return n.indexOf("_")!==-1&&(n=n.replace(/_/g,"")),r=n[0],(r==="-"||r==="+")&&(r==="-"&&(t=-1),n=n.slice(1),r=n[0]),n==="0"?0:r==="0"?n[1]==="b"?t*parseInt(n.slice(2),2):n[1]==="x"?t*parseInt(n,16):t*parseInt(n,8):n.indexOf(":")!==-1?(n.split(":").forEach(function(o){s.unshift(parseInt(o,10))}),n=0,i=1,s.forEach(function(o){n+=o*i,i*=60}),t*n):t*parseInt(n,10)}function ys(e){return Object.prototype.toString.call(e)==="[object Number]"&&e%1===0&&!hs.isNegativeZero(e)}var bs=new ds("tag:yaml.org,2002:int",{kind:"scalar",resolve:ms,construct:gs,predicate:ys,represent:{binary:function(e){return e>=0?"0b"+e.toString(2):"-0b"+e.toString(2).slice(1)},octal:function(e){return e>=0?"0"+e.toString(8):"-0"+e.toString(8).slice(1)},decimal:function(e){return e.toString(10)},hexadecimal:function(e){return e>=0?"0x"+e.toString(16).toUpperCase():"-0x"+e.toString(16).toUpperCase().slice(1)}},defaultStyle:"decimal",styleAliases:{binary:[2,"bin"],octal:[8,"oct"],decimal:[10,"dec"],hexadecimal:[16,"hex"]}}),yt=B,_s=L,vs=new RegExp("^(?:[-+]?(?:0|[1-9][0-9_]*)(?:\\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?|\\.[0-9_]+(?:[eE][-+]?[0-9]+)?|[-+]?[0-9][0-9_]*(?::[0-5]?[0-9])+\\.[0-9_]*|[-+]?\\.(?:inf|Inf|INF)|\\.(?:nan|NaN|NAN))$");function ws(e){return!(e===null||!vs.test(e)||e[e.length-1]==="_")}function ks(e){var n,t,r,i;return n=e.replace(/_/g,"").toLowerCase(),t=n[0]==="-"?-1:1,i=[],"+-".indexOf(n[0])>=0&&(n=n.slice(1)),n===".inf"?t===1?Number.POSITIVE_INFINITY:Number.NEGATIVE_INFINITY:n===".nan"?NaN:n.indexOf(":")>=0?(n.split(":").forEach(function(s){i.unshift(parseFloat(s,10))}),n=0,r=1,i.forEach(function(s){n+=s*r,r*=60}),t*n):t*parseFloat(n,10)}var xs=/^[-+]?[0-9]+e/;function Cs(e,n){var t;if(isNaN(e))switch(n){case"lowercase":return".nan";case"uppercase":return".NAN";case"camelcase":return".NaN"}else if(Number.POSITIVE_INFINITY===e)switch(n){case"lowercase":return".inf";case"uppercase":return".INF";case"camelcase":return".Inf"}else if(Number.NEGATIVE_INFINITY===e)switch(n){case"lowercase":return"-.inf";case"uppercase":return"-.INF";case"camelcase":return"-.Inf"}else if(yt.isNegativeZero(e))return"-0.0";return t=e.toString(10),xs.test(t)?t.replace("e",".e"):t}function Ts(e){return Object.prototype.toString.call(e)==="[object Number]"&&(e%1!==0||yt.isNegativeZero(e))}var As=new _s("tag:yaml.org,2002:float",{kind:"scalar",resolve:ws,construct:ks,predicate:Ts,represent:Cs,defaultStyle:"lowercase"}),Ss=he,bt=new Ss({include:[dn],implicit:[rs,cs,bs,As]}),Is=he,_t=new Is({include:[bt]}),Es=L,vt=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9])-([0-9][0-9])$"),wt=new RegExp("^([0-9][0-9][0-9][0-9])-([0-9][0-9]?)-([0-9][0-9]?)(?:[Tt]|[ \\t]+)([0-9][0-9]?):([0-9][0-9]):([0-9][0-9])(?:\\.([0-9]*))?(?:[ \\t]*(Z|([-+])([0-9][0-9]?)(?::([0-9][0-9]))?))?$");function Ms(e){return e===null?!1:vt.exec(e)!==null||wt.exec(e)!==null}function Ls(e){var n,t,r,i,s,o,a,l=0,c=null,h,d,u;if(n=vt.exec(e),n===null&&(n=wt.exec(e)),n===null)throw new Error("Date resolve error");if(t=+n[1],r=+n[2]-1,i=+n[3],!n[4])return new Date(Date.UTC(t,r,i));if(s=+n[4],o=+n[5],a=+n[6],n[7]){for(l=n[7].slice(0,3);l.length<3;)l+="0";l=+l}return n[9]&&(h=+n[10],d=+(n[11]||0),c=(h*60+d)*6e4,n[9]==="-"&&(c=-c)),u=new Date(Date.UTC(t,r,i,s,o,a,l)),c&&u.setTime(u.getTime()-c),u}function Rs(e){return e.toISOString()}var Os=new Es("tag:yaml.org,2002:timestamp",{kind:"scalar",resolve:Ms,construct:Ls,instanceOf:Date,represent:Rs}),Ns=L;function Fs(e){return e==="<<"||e===null}var Ds=new Ns("tag:yaml.org,2002:merge",{kind:"scalar",resolve:Fs});function kt(e){throw new Error('Could not dynamically require "'+e+'". Please configure the dynamicRequireTargets or/and ignoreDynamicRequires option of @rollup/plugin-commonjs appropriately for this require call to work.')}var Q;try{var Ps=kt;Q=Ps("buffer").Buffer}catch{}var Us=L,un=`ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=
\r`;function Bs(e){if(e===null)return!1;var n,t,r=0,i=e.length,s=un;for(t=0;t<i;t++)if(n=s.indexOf(e.charAt(t)),!(n>64)){if(n<0)return!1;r+=6}return r%8===0}function Hs(e){var n,t,r=e.replace(/[\r\n=]/g,""),i=r.length,s=un,o=0,a=[];for(n=0;n<i;n++)n%4===0&&n&&(a.push(o>>16&255),a.push(o>>8&255),a.push(o&255)),o=o<<6|s.indexOf(r.charAt(n));return t=i%4*6,t===0?(a.push(o>>16&255),a.push(o>>8&255),a.push(o&255)):t===18?(a.push(o>>10&255),a.push(o>>2&255)):t===12&&a.push(o>>4&255),Q?Q.from?Q.from(a):new Q(a):a}function $s(e){var n="",t=0,r,i,s=e.length,o=un;for(r=0;r<s;r++)r%3===0&&r&&(n+=o[t>>18&63],n+=o[t>>12&63],n+=o[t>>6&63],n+=o[t&63]),t=(t<<8)+e[r];return i=s%3,i===0?(n+=o[t>>18&63],n+=o[t>>12&63],n+=o[t>>6&63],n+=o[t&63]):i===2?(n+=o[t>>10&63],n+=o[t>>4&63],n+=o[t<<2&63],n+=o[64]):i===1&&(n+=o[t>>2&63],n+=o[t<<4&63],n+=o[64],n+=o[64]),n}function Gs(e){return Q&&Q.isBuffer(e)}var Ws=new Us("tag:yaml.org,2002:binary",{kind:"scalar",resolve:Bs,construct:Hs,predicate:Gs,represent:$s}),js=L,Ys=Object.prototype.hasOwnProperty,Ks=Object.prototype.toString;function qs(e){if(e===null)return!0;var n=[],t,r,i,s,o,a=e;for(t=0,r=a.length;t<r;t+=1){if(i=a[t],o=!1,Ks.call(i)!=="[object Object]")return!1;for(s in i)if(Ys.call(i,s))if(!o)o=!0;else return!1;if(!o)return!1;if(n.indexOf(s)===-1)n.push(s);else return!1}return!0}function Vs(e){return e!==null?e:[]}var zs=new js("tag:yaml.org,2002:omap",{kind:"sequence",resolve:qs,construct:Vs}),Xs=L,Js=Object.prototype.toString;function Qs(e){if(e===null)return!0;var n,t,r,i,s,o=e;for(s=new Array(o.length),n=0,t=o.length;n<t;n+=1){if(r=o[n],Js.call(r)!=="[object Object]"||(i=Object.keys(r),i.length!==1))return!1;s[n]=[i[0],r[i[0]]]}return!0}function Zs(e){if(e===null)return[];var n,t,r,i,s,o=e;for(s=new Array(o.length),n=0,t=o.length;n<t;n+=1)r=o[n],i=Object.keys(r),s[n]=[i[0],r[i[0]]];return s}var ea=new Xs("tag:yaml.org,2002:pairs",{kind:"sequence",resolve:Qs,construct:Zs}),na=L,ta=Object.prototype.hasOwnProperty;function ia(e){if(e===null)return!0;var n,t=e;for(n in t)if(ta.call(t,n)&&t[n]!==null)return!1;return!0}function ra(e){return e!==null?e:{}}var oa=new na("tag:yaml.org,2002:set",{kind:"mapping",resolve:ia,construct:ra}),sa=he,Ce=new sa({include:[_t],implicit:[Os,Ds],explicit:[Ws,zs,ea,oa]}),aa=L;function la(){return!0}function ca(){}function ha(){return""}function da(e){return typeof e>"u"}var ua=new aa("tag:yaml.org,2002:js/undefined",{kind:"scalar",resolve:la,construct:ca,predicate:da,represent:ha}),pa=L;function fa(e){if(e===null||e.length===0)return!1;var n=e,t=/\/([gim]*)$/.exec(e),r="";return!(n[0]==="/"&&(t&&(r=t[1]),r.length>3||n[n.length-r.length-1]!=="/"))}function ma(e){var n=e,t=/\/([gim]*)$/.exec(e),r="";return n[0]==="/"&&(t&&(r=t[1]),n=n.slice(1,n.length-r.length-1)),new RegExp(n,r)}function ga(e){var n="/"+e.source+"/";return e.global&&(n+="g"),e.multiline&&(n+="m"),e.ignoreCase&&(n+="i"),n}function ya(e){return Object.prototype.toString.call(e)==="[object RegExp]"}var ba=new pa("tag:yaml.org,2002:js/regexp",{kind:"scalar",resolve:fa,construct:ma,predicate:ya,represent:ga}),Pe;try{var _a=kt;Pe=_a("esprima")}catch{typeof window<"u"&&(Pe=window.esprima)}var va=L;function wa(e){if(e===null)return!1;try{var n="("+e+")",t=Pe.parse(n,{range:!0});return!(t.type!=="Program"||t.body.length!==1||t.body[0].type!=="ExpressionStatement"||t.body[0].expression.type!=="ArrowFunctionExpression"&&t.body[0].expression.type!=="FunctionExpression")}catch{return!1}}function ka(e){var n="("+e+")",t=Pe.parse(n,{range:!0}),r=[],i;if(t.type!=="Program"||t.body.length!==1||t.body[0].type!=="ExpressionStatement"||t.body[0].expression.type!=="ArrowFunctionExpression"&&t.body[0].expression.type!=="FunctionExpression")throw new Error("Failed to resolve function");return t.body[0].expression.params.forEach(function(s){r.push(s.name)}),i=t.body[0].expression.body.range,t.body[0].expression.body.type==="BlockStatement"?new Function(r,n.slice(i[0]+1,i[1]-1)):new Function(r,"return "+n.slice(i[0],i[1]))}function xa(e){return e.toString()}function Ca(e){return Object.prototype.toString.call(e)==="[object Function]"}var Ta=new va("tag:yaml.org,2002:js/function",{kind:"scalar",resolve:wa,construct:ka,predicate:Ca,represent:xa}),Xn=he,$e=Xn.DEFAULT=new Xn({include:[Ce],explicit:[ua,ba,Ta]}),j=B,xt=xe,Aa=Ho,Ct=Ce,Sa=$e,V=Object.prototype.hasOwnProperty,Ue=1,Tt=2,At=3,Be=4,nn=1,Ia=2,Jn=3,Ea=/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x84\x86-\x9F\uFFFE\uFFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF]/,Ma=/[\x85\u2028\u2029]/,La=/[,\[\]\{\}]/,St=/^(?:!|!!|![a-z\-]+!)$/i,It=/^(?:!|[^,\[\]\{\}])(?:%[0-9a-f]{2}|[0-9a-z\-#;\/\?:@&=\+\$,_\.!~\*'\(\)\[\]])*$/i;function Qn(e){return Object.prototype.toString.call(e)}function G(e){return e===10||e===13}function Z(e){return e===9||e===32}function F(e){return e===9||e===32||e===10||e===13}function oe(e){return e===44||e===91||e===93||e===123||e===125}function Ra(e){var n;return 48<=e&&e<=57?e-48:(n=e|32,97<=n&&n<=102?n-97+10:-1)}function Oa(e){return e===120?2:e===117?4:e===85?8:0}function Na(e){return 48<=e&&e<=57?e-48:-1}function Zn(e){return e===48?"\0":e===97?"\x07":e===98?"\b":e===116||e===9?"	":e===110?`
`:e===118?"\v":e===102?"\f":e===114?"\r":e===101?"\x1B":e===32?" ":e===34?'"':e===47?"/":e===92?"\\":e===78?"":e===95?" ":e===76?"\u2028":e===80?"\u2029":""}function Fa(e){return e<=65535?String.fromCharCode(e):String.fromCharCode((e-65536>>10)+55296,(e-65536&1023)+56320)}function Et(e,n,t){n==="__proto__"?Object.defineProperty(e,n,{configurable:!0,enumerable:!0,writable:!0,value:t}):e[n]=t}var Mt=new Array(256),Lt=new Array(256);for(var ie=0;ie<256;ie++)Mt[ie]=Zn(ie)?1:0,Lt[ie]=Zn(ie);function Da(e,n){this.input=e,this.filename=n.filename||null,this.schema=n.schema||Sa,this.onWarning=n.onWarning||null,this.legacy=n.legacy||!1,this.json=n.json||!1,this.listener=n.listener||null,this.implicitTypes=this.schema.compiledImplicit,this.typeMap=this.schema.compiledTypeMap,this.length=e.length,this.position=0,this.line=0,this.lineStart=0,this.lineIndent=0,this.documents=[]}function Rt(e,n){return new xt(n,new Aa(e.filename,e.input,e.position,e.line,e.position-e.lineStart))}function _(e,n){throw Rt(e,n)}function He(e,n){e.onWarning&&e.onWarning.call(null,Rt(e,n))}var et={YAML:function(n,t,r){var i,s,o;n.version!==null&&_(n,"duplication of %YAML directive"),r.length!==1&&_(n,"YAML directive accepts exactly one argument"),i=/^([0-9]+)\.([0-9]+)$/.exec(r[0]),i===null&&_(n,"ill-formed argument of the YAML directive"),s=parseInt(i[1],10),o=parseInt(i[2],10),s!==1&&_(n,"unacceptable YAML version of the document"),n.version=r[0],n.checkLineBreaks=o<2,o!==1&&o!==2&&He(n,"unsupported YAML version of the document")},TAG:function(n,t,r){var i,s;r.length!==2&&_(n,"TAG directive accepts exactly two arguments"),i=r[0],s=r[1],St.test(i)||_(n,"ill-formed tag handle (first argument) of the TAG directive"),V.call(n.tagMap,i)&&_(n,'there is a previously declared suffix for "'+i+'" tag handle'),It.test(s)||_(n,"ill-formed tag prefix (second argument) of the TAG directive"),n.tagMap[i]=s}};function q(e,n,t,r){var i,s,o,a;if(n<t){if(a=e.input.slice(n,t),r)for(i=0,s=a.length;i<s;i+=1)o=a.charCodeAt(i),o===9||32<=o&&o<=1114111||_(e,"expected valid JSON character");else Ea.test(a)&&_(e,"the stream contains non-printable characters");e.result+=a}}function nt(e,n,t,r){var i,s,o,a;for(j.isObject(t)||_(e,"cannot merge mappings; the provided source object is unacceptable"),i=Object.keys(t),o=0,a=i.length;o<a;o+=1)s=i[o],V.call(n,s)||(Et(n,s,t[s]),r[s]=!0)}function se(e,n,t,r,i,s,o,a){var l,c;if(Array.isArray(i))for(i=Array.prototype.slice.call(i),l=0,c=i.length;l<c;l+=1)Array.isArray(i[l])&&_(e,"nested arrays are not supported inside keys"),typeof i=="object"&&Qn(i[l])==="[object Object]"&&(i[l]="[object Object]");if(typeof i=="object"&&Qn(i)==="[object Object]"&&(i="[object Object]"),i=String(i),n===null&&(n={}),r==="tag:yaml.org,2002:merge")if(Array.isArray(s))for(l=0,c=s.length;l<c;l+=1)nt(e,n,s[l],t);else nt(e,n,s,t);else!e.json&&!V.call(t,i)&&V.call(n,i)&&(e.line=o||e.line,e.position=a||e.position,_(e,"duplicated mapping key")),Et(n,i,s),delete t[i];return n}function pn(e){var n;n=e.input.charCodeAt(e.position),n===10?e.position++:n===13?(e.position++,e.input.charCodeAt(e.position)===10&&e.position++):_(e,"a line break is expected"),e.line+=1,e.lineStart=e.position}function I(e,n,t){for(var r=0,i=e.input.charCodeAt(e.position);i!==0;){for(;Z(i);)i=e.input.charCodeAt(++e.position);if(n&&i===35)do i=e.input.charCodeAt(++e.position);while(i!==10&&i!==13&&i!==0);if(G(i))for(pn(e),i=e.input.charCodeAt(e.position),r++,e.lineIndent=0;i===32;)e.lineIndent++,i=e.input.charCodeAt(++e.position);else break}return t!==-1&&r!==0&&e.lineIndent<t&&He(e,"deficient indentation"),r}function Ge(e){var n=e.position,t;return t=e.input.charCodeAt(n),!!((t===45||t===46)&&t===e.input.charCodeAt(n+1)&&t===e.input.charCodeAt(n+2)&&(n+=3,t=e.input.charCodeAt(n),t===0||F(t)))}function fn(e,n){n===1?e.result+=" ":n>1&&(e.result+=j.repeat(`
`,n-1))}function Pa(e,n,t){var r,i,s,o,a,l,c,h,d=e.kind,u=e.result,p;if(p=e.input.charCodeAt(e.position),F(p)||oe(p)||p===35||p===38||p===42||p===33||p===124||p===62||p===39||p===34||p===37||p===64||p===96||(p===63||p===45)&&(i=e.input.charCodeAt(e.position+1),F(i)||t&&oe(i)))return!1;for(e.kind="scalar",e.result="",s=o=e.position,a=!1;p!==0;){if(p===58){if(i=e.input.charCodeAt(e.position+1),F(i)||t&&oe(i))break}else if(p===35){if(r=e.input.charCodeAt(e.position-1),F(r))break}else{if(e.position===e.lineStart&&Ge(e)||t&&oe(p))break;if(G(p))if(l=e.line,c=e.lineStart,h=e.lineIndent,I(e,!1,-1),e.lineIndent>=n){a=!0,p=e.input.charCodeAt(e.position);continue}else{e.position=o,e.line=l,e.lineStart=c,e.lineIndent=h;break}}a&&(q(e,s,o,!1),fn(e,e.line-l),s=o=e.position,a=!1),Z(p)||(o=e.position+1),p=e.input.charCodeAt(++e.position)}return q(e,s,o,!1),e.result?!0:(e.kind=d,e.result=u,!1)}function Ua(e,n){var t,r,i;if(t=e.input.charCodeAt(e.position),t!==39)return!1;for(e.kind="scalar",e.result="",e.position++,r=i=e.position;(t=e.input.charCodeAt(e.position))!==0;)if(t===39)if(q(e,r,e.position,!0),t=e.input.charCodeAt(++e.position),t===39)r=e.position,e.position++,i=e.position;else return!0;else G(t)?(q(e,r,i,!0),fn(e,I(e,!1,n)),r=i=e.position):e.position===e.lineStart&&Ge(e)?_(e,"unexpected end of the document within a single quoted scalar"):(e.position++,i=e.position);_(e,"unexpected end of the stream within a single quoted scalar")}function Ba(e,n){var t,r,i,s,o,a;if(a=e.input.charCodeAt(e.position),a!==34)return!1;for(e.kind="scalar",e.result="",e.position++,t=r=e.position;(a=e.input.charCodeAt(e.position))!==0;){if(a===34)return q(e,t,e.position,!0),e.position++,!0;if(a===92){if(q(e,t,e.position,!0),a=e.input.charCodeAt(++e.position),G(a))I(e,!1,n);else if(a<256&&Mt[a])e.result+=Lt[a],e.position++;else if((o=Oa(a))>0){for(i=o,s=0;i>0;i--)a=e.input.charCodeAt(++e.position),(o=Ra(a))>=0?s=(s<<4)+o:_(e,"expected hexadecimal character");e.result+=Fa(s),e.position++}else _(e,"unknown escape sequence");t=r=e.position}else G(a)?(q(e,t,r,!0),fn(e,I(e,!1,n)),t=r=e.position):e.position===e.lineStart&&Ge(e)?_(e,"unexpected end of the document within a double quoted scalar"):(e.position++,r=e.position)}_(e,"unexpected end of the stream within a double quoted scalar")}function Ha(e,n){var t=!0,r,i=e.tag,s,o=e.anchor,a,l,c,h,d,u={},p,m,y,g;if(g=e.input.charCodeAt(e.position),g===91)l=93,d=!1,s=[];else if(g===123)l=125,d=!0,s={};else return!1;for(e.anchor!==null&&(e.anchorMap[e.anchor]=s),g=e.input.charCodeAt(++e.position);g!==0;){if(I(e,!0,n),g=e.input.charCodeAt(e.position),g===l)return e.position++,e.tag=i,e.anchor=o,e.kind=d?"mapping":"sequence",e.result=s,!0;t||_(e,"missed comma between flow collection entries"),m=p=y=null,c=h=!1,g===63&&(a=e.input.charCodeAt(e.position+1),F(a)&&(c=h=!0,e.position++,I(e,!0,n))),r=e.line,le(e,n,Ue,!1,!0),m=e.tag,p=e.result,I(e,!0,n),g=e.input.charCodeAt(e.position),(h||e.line===r)&&g===58&&(c=!0,g=e.input.charCodeAt(++e.position),I(e,!0,n),le(e,n,Ue,!1,!0),y=e.result),d?se(e,s,u,m,p,y):c?s.push(se(e,null,u,m,p,y)):s.push(p),I(e,!0,n),g=e.input.charCodeAt(e.position),g===44?(t=!0,g=e.input.charCodeAt(++e.position)):t=!1}_(e,"unexpected end of the stream within a flow collection")}function $a(e,n){var t,r,i=nn,s=!1,o=!1,a=n,l=0,c=!1,h,d;if(d=e.input.charCodeAt(e.position),d===124)r=!1;else if(d===62)r=!0;else return!1;for(e.kind="scalar",e.result="";d!==0;)if(d=e.input.charCodeAt(++e.position),d===43||d===45)nn===i?i=d===43?Jn:Ia:_(e,"repeat of a chomping mode identifier");else if((h=Na(d))>=0)h===0?_(e,"bad explicit indentation width of a block scalar; it cannot be less than one"):o?_(e,"repeat of an indentation width identifier"):(a=n+h-1,o=!0);else break;if(Z(d)){do d=e.input.charCodeAt(++e.position);while(Z(d));if(d===35)do d=e.input.charCodeAt(++e.position);while(!G(d)&&d!==0)}for(;d!==0;){for(pn(e),e.lineIndent=0,d=e.input.charCodeAt(e.position);(!o||e.lineIndent<a)&&d===32;)e.lineIndent++,d=e.input.charCodeAt(++e.position);if(!o&&e.lineIndent>a&&(a=e.lineIndent),G(d)){l++;continue}if(e.lineIndent<a){i===Jn?e.result+=j.repeat(`
`,s?1+l:l):i===nn&&s&&(e.result+=`
`);break}for(r?Z(d)?(c=!0,e.result+=j.repeat(`
`,s?1+l:l)):c?(c=!1,e.result+=j.repeat(`
`,l+1)):l===0?s&&(e.result+=" "):e.result+=j.repeat(`
`,l):e.result+=j.repeat(`
`,s?1+l:l),s=!0,o=!0,l=0,t=e.position;!G(d)&&d!==0;)d=e.input.charCodeAt(++e.position);q(e,t,e.position,!1)}return!0}function tt(e,n){var t,r=e.tag,i=e.anchor,s=[],o,a=!1,l;for(e.anchor!==null&&(e.anchorMap[e.anchor]=s),l=e.input.charCodeAt(e.position);l!==0&&!(l!==45||(o=e.input.charCodeAt(e.position+1),!F(o)));){if(a=!0,e.position++,I(e,!0,-1)&&e.lineIndent<=n){s.push(null),l=e.input.charCodeAt(e.position);continue}if(t=e.line,le(e,n,At,!1,!0),s.push(e.result),I(e,!0,-1),l=e.input.charCodeAt(e.position),(e.line===t||e.lineIndent>n)&&l!==0)_(e,"bad indentation of a sequence entry");else if(e.lineIndent<n)break}return a?(e.tag=r,e.anchor=i,e.kind="sequence",e.result=s,!0):!1}function Ga(e,n,t){var r,i,s,o,a=e.tag,l=e.anchor,c={},h={},d=null,u=null,p=null,m=!1,y=!1,g;for(e.anchor!==null&&(e.anchorMap[e.anchor]=c),g=e.input.charCodeAt(e.position);g!==0;){if(r=e.input.charCodeAt(e.position+1),s=e.line,o=e.position,(g===63||g===58)&&F(r))g===63?(m&&(se(e,c,h,d,u,null),d=u=p=null),y=!0,m=!0,i=!0):m?(m=!1,i=!0):_(e,"incomplete explicit mapping pair; a key node is missed; or followed by a non-tabulated empty line"),e.position+=1,g=r;else if(le(e,t,Tt,!1,!0))if(e.line===s){for(g=e.input.charCodeAt(e.position);Z(g);)g=e.input.charCodeAt(++e.position);if(g===58)g=e.input.charCodeAt(++e.position),F(g)||_(e,"a whitespace character is expected after the key-value separator within a block mapping"),m&&(se(e,c,h,d,u,null),d=u=p=null),y=!0,m=!1,i=!1,d=e.tag,u=e.result;else if(y)_(e,"can not read an implicit mapping pair; a colon is missed");else return e.tag=a,e.anchor=l,!0}else if(y)_(e,"can not read a block mapping entry; a multiline key may not be an implicit key");else return e.tag=a,e.anchor=l,!0;else break;if((e.line===s||e.lineIndent>n)&&(le(e,n,Be,!0,i)&&(m?u=e.result:p=e.result),m||(se(e,c,h,d,u,p,s,o),d=u=p=null),I(e,!0,-1),g=e.input.charCodeAt(e.position)),e.lineIndent>n&&g!==0)_(e,"bad indentation of a mapping entry");else if(e.lineIndent<n)break}return m&&se(e,c,h,d,u,null),y&&(e.tag=a,e.anchor=l,e.kind="mapping",e.result=c),y}function Wa(e){var n,t=!1,r=!1,i,s,o;if(o=e.input.charCodeAt(e.position),o!==33)return!1;if(e.tag!==null&&_(e,"duplication of a tag property"),o=e.input.charCodeAt(++e.position),o===60?(t=!0,o=e.input.charCodeAt(++e.position)):o===33?(r=!0,i="!!",o=e.input.charCodeAt(++e.position)):i="!",n=e.position,t){do o=e.input.charCodeAt(++e.position);while(o!==0&&o!==62);e.position<e.length?(s=e.input.slice(n,e.position),o=e.input.charCodeAt(++e.position)):_(e,"unexpected end of the stream within a verbatim tag")}else{for(;o!==0&&!F(o);)o===33&&(r?_(e,"tag suffix cannot contain exclamation marks"):(i=e.input.slice(n-1,e.position+1),St.test(i)||_(e,"named tag handle cannot contain such characters"),r=!0,n=e.position+1)),o=e.input.charCodeAt(++e.position);s=e.input.slice(n,e.position),La.test(s)&&_(e,"tag suffix cannot contain flow indicator characters")}return s&&!It.test(s)&&_(e,"tag name cannot contain such characters: "+s),t?e.tag=s:V.call(e.tagMap,i)?e.tag=e.tagMap[i]+s:i==="!"?e.tag="!"+s:i==="!!"?e.tag="tag:yaml.org,2002:"+s:_(e,'undeclared tag handle "'+i+'"'),!0}function ja(e){var n,t;if(t=e.input.charCodeAt(e.position),t!==38)return!1;for(e.anchor!==null&&_(e,"duplication of an anchor property"),t=e.input.charCodeAt(++e.position),n=e.position;t!==0&&!F(t)&&!oe(t);)t=e.input.charCodeAt(++e.position);return e.position===n&&_(e,"name of an anchor node must contain at least one character"),e.anchor=e.input.slice(n,e.position),!0}function Ya(e){var n,t,r;if(r=e.input.charCodeAt(e.position),r!==42)return!1;for(r=e.input.charCodeAt(++e.position),n=e.position;r!==0&&!F(r)&&!oe(r);)r=e.input.charCodeAt(++e.position);return e.position===n&&_(e,"name of an alias node must contain at least one character"),t=e.input.slice(n,e.position),V.call(e.anchorMap,t)||_(e,'unidentified alias "'+t+'"'),e.result=e.anchorMap[t],I(e,!0,-1),!0}function le(e,n,t,r,i){var s,o,a,l=1,c=!1,h=!1,d,u,p,m,y;if(e.listener!==null&&e.listener("open",e),e.tag=null,e.anchor=null,e.kind=null,e.result=null,s=o=a=Be===t||At===t,r&&I(e,!0,-1)&&(c=!0,e.lineIndent>n?l=1:e.lineIndent===n?l=0:e.lineIndent<n&&(l=-1)),l===1)for(;Wa(e)||ja(e);)I(e,!0,-1)?(c=!0,a=s,e.lineIndent>n?l=1:e.lineIndent===n?l=0:e.lineIndent<n&&(l=-1)):a=!1;if(a&&(a=c||i),(l===1||Be===t)&&(Ue===t||Tt===t?m=n:m=n+1,y=e.position-e.lineStart,l===1?a&&(tt(e,y)||Ga(e,y,m))||Ha(e,m)?h=!0:(o&&$a(e,m)||Ua(e,m)||Ba(e,m)?h=!0:Ya(e)?(h=!0,(e.tag!==null||e.anchor!==null)&&_(e,"alias node should not have any properties")):Pa(e,m,Ue===t)&&(h=!0,e.tag===null&&(e.tag="?")),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):l===0&&(h=a&&tt(e,y))),e.tag!==null&&e.tag!=="!")if(e.tag==="?"){for(e.result!==null&&e.kind!=="scalar"&&_(e,'unacceptable node kind for !<?> tag; it should be "scalar", not "'+e.kind+'"'),d=0,u=e.implicitTypes.length;d<u;d+=1)if(p=e.implicitTypes[d],p.resolve(e.result)){e.result=p.construct(e.result),e.tag=p.tag,e.anchor!==null&&(e.anchorMap[e.anchor]=e.result);break}}else V.call(e.typeMap[e.kind||"fallback"],e.tag)?(p=e.typeMap[e.kind||"fallback"][e.tag],e.result!==null&&p.kind!==e.kind&&_(e,"unacceptable node kind for !<"+e.tag+'> tag; it should be "'+p.kind+'", not "'+e.kind+'"'),p.resolve(e.result)?(e.result=p.construct(e.result),e.anchor!==null&&(e.anchorMap[e.anchor]=e.result)):_(e,"cannot resolve a node with !<"+e.tag+"> explicit tag")):_(e,"unknown tag !<"+e.tag+">");return e.listener!==null&&e.listener("close",e),e.tag!==null||e.anchor!==null||h}function Ka(e){var n=e.position,t,r,i,s=!1,o;for(e.version=null,e.checkLineBreaks=e.legacy,e.tagMap={},e.anchorMap={};(o=e.input.charCodeAt(e.position))!==0&&(I(e,!0,-1),o=e.input.charCodeAt(e.position),!(e.lineIndent>0||o!==37));){for(s=!0,o=e.input.charCodeAt(++e.position),t=e.position;o!==0&&!F(o);)o=e.input.charCodeAt(++e.position);for(r=e.input.slice(t,e.position),i=[],r.length<1&&_(e,"directive name must not be less than one character in length");o!==0;){for(;Z(o);)o=e.input.charCodeAt(++e.position);if(o===35){do o=e.input.charCodeAt(++e.position);while(o!==0&&!G(o));break}if(G(o))break;for(t=e.position;o!==0&&!F(o);)o=e.input.charCodeAt(++e.position);i.push(e.input.slice(t,e.position))}o!==0&&pn(e),V.call(et,r)?et[r](e,r,i):He(e,'unknown document directive "'+r+'"')}if(I(e,!0,-1),e.lineIndent===0&&e.input.charCodeAt(e.position)===45&&e.input.charCodeAt(e.position+1)===45&&e.input.charCodeAt(e.position+2)===45?(e.position+=3,I(e,!0,-1)):s&&_(e,"directives end mark is expected"),le(e,e.lineIndent-1,Be,!1,!0),I(e,!0,-1),e.checkLineBreaks&&Ma.test(e.input.slice(n,e.position))&&He(e,"non-ASCII line breaks are interpreted as content"),e.documents.push(e.result),e.position===e.lineStart&&Ge(e)){e.input.charCodeAt(e.position)===46&&(e.position+=3,I(e,!0,-1));return}if(e.position<e.length-1)_(e,"end of the stream or a document separator is expected");else return}function Ot(e,n){e=String(e),n=n||{},e.length!==0&&(e.charCodeAt(e.length-1)!==10&&e.charCodeAt(e.length-1)!==13&&(e+=`
`),e.charCodeAt(0)===65279&&(e=e.slice(1)));var t=new Da(e,n),r=e.indexOf("\0");for(r!==-1&&(t.position=r,_(t,"null byte is not allowed in input")),t.input+="\0";t.input.charCodeAt(t.position)===32;)t.lineIndent+=1,t.position+=1;for(;t.position<t.length-1;)Ka(t);return t.documents}function Nt(e,n,t){n!==null&&typeof n=="object"&&typeof t>"u"&&(t=n,n=null);var r=Ot(e,t);if(typeof n!="function")return r;for(var i=0,s=r.length;i<s;i+=1)n(r[i])}function Ft(e,n){var t=Ot(e,n);if(t.length!==0){if(t.length===1)return t[0];throw new xt("expected a single document in the stream, but found more")}}function qa(e,n,t){return typeof n=="object"&&n!==null&&typeof t>"u"&&(t=n,n=null),Nt(e,n,j.extend({schema:Ct},t))}function Va(e,n){return Ft(e,j.extend({schema:Ct},n))}ke.loadAll=Nt;ke.load=Ft;ke.safeLoadAll=qa;ke.safeLoad=Va;var mn={},Te=B,Ae=xe,za=$e,Xa=Ce,Dt=Object.prototype.toString,Pt=Object.prototype.hasOwnProperty,Ja=9,_e=10,Qa=13,Za=32,el=33,nl=34,Ut=35,tl=37,il=38,rl=39,ol=42,Bt=44,sl=45,Ht=58,al=61,ll=62,cl=63,hl=64,$t=91,Gt=93,dl=96,Wt=123,ul=124,jt=125,O={};O[0]="\\0";O[7]="\\a";O[8]="\\b";O[9]="\\t";O[10]="\\n";O[11]="\\v";O[12]="\\f";O[13]="\\r";O[27]="\\e";O[34]='\\"';O[92]="\\\\";O[133]="\\N";O[160]="\\_";O[8232]="\\L";O[8233]="\\P";var pl=["y","Y","yes","Yes","YES","on","On","ON","n","N","no","No","NO","off","Off","OFF"];function fl(e,n){var t,r,i,s,o,a,l;if(n===null)return{};for(t={},r=Object.keys(n),i=0,s=r.length;i<s;i+=1)o=r[i],a=String(n[o]),o.slice(0,2)==="!!"&&(o="tag:yaml.org,2002:"+o.slice(2)),l=e.compiledTypeMap.fallback[o],l&&Pt.call(l.styleAliases,a)&&(a=l.styleAliases[a]),t[o]=a;return t}function it(e){var n,t,r;if(n=e.toString(16).toUpperCase(),e<=255)t="x",r=2;else if(e<=65535)t="u",r=4;else if(e<=4294967295)t="U",r=8;else throw new Ae("code point within a string may not be greater than 0xFFFFFFFF");return"\\"+t+Te.repeat("0",r-n.length)+n}function ml(e){this.schema=e.schema||za,this.indent=Math.max(1,e.indent||2),this.noArrayIndent=e.noArrayIndent||!1,this.skipInvalid=e.skipInvalid||!1,this.flowLevel=Te.isNothing(e.flowLevel)?-1:e.flowLevel,this.styleMap=fl(this.schema,e.styles||null),this.sortKeys=e.sortKeys||!1,this.lineWidth=e.lineWidth||80,this.noRefs=e.noRefs||!1,this.noCompatMode=e.noCompatMode||!1,this.condenseFlow=e.condenseFlow||!1,this.implicitTypes=this.schema.compiledImplicit,this.explicitTypes=this.schema.compiledExplicit,this.tag=null,this.result="",this.duplicates=[],this.usedDuplicates=null}function rt(e,n){for(var t=Te.repeat(" ",n),r=0,i=-1,s="",o,a=e.length;r<a;)i=e.indexOf(`
`,r),i===-1?(o=e.slice(r),r=a):(o=e.slice(r,i+1),r=i+1),o.length&&o!==`
`&&(s+=t),s+=o;return s}function sn(e,n){return`
`+Te.repeat(" ",e.indent*n)}function gl(e,n){var t,r,i;for(t=0,r=e.implicitTypes.length;t<r;t+=1)if(i=e.implicitTypes[t],i.resolve(n))return!0;return!1}function gn(e){return e===Za||e===Ja}function ce(e){return 32<=e&&e<=126||161<=e&&e<=55295&&e!==8232&&e!==8233||57344<=e&&e<=65533&&e!==65279||65536<=e&&e<=1114111}function yl(e){return ce(e)&&!gn(e)&&e!==65279&&e!==Qa&&e!==_e}function ot(e,n){return ce(e)&&e!==65279&&e!==Bt&&e!==$t&&e!==Gt&&e!==Wt&&e!==jt&&e!==Ht&&(e!==Ut||n&&yl(n))}function bl(e){return ce(e)&&e!==65279&&!gn(e)&&e!==sl&&e!==cl&&e!==Ht&&e!==Bt&&e!==$t&&e!==Gt&&e!==Wt&&e!==jt&&e!==Ut&&e!==il&&e!==ol&&e!==el&&e!==ul&&e!==al&&e!==ll&&e!==rl&&e!==nl&&e!==tl&&e!==hl&&e!==dl}function Yt(e){var n=/^\n* /;return n.test(e)}var Kt=1,qt=2,Vt=3,zt=4,Fe=5;function _l(e,n,t,r,i){var s,o,a,l=!1,c=!1,h=r!==-1,d=-1,u=bl(e.charCodeAt(0))&&!gn(e.charCodeAt(e.length-1));if(n)for(s=0;s<e.length;s++){if(o=e.charCodeAt(s),!ce(o))return Fe;a=s>0?e.charCodeAt(s-1):null,u=u&&ot(o,a)}else{for(s=0;s<e.length;s++){if(o=e.charCodeAt(s),o===_e)l=!0,h&&(c=c||s-d-1>r&&e[d+1]!==" ",d=s);else if(!ce(o))return Fe;a=s>0?e.charCodeAt(s-1):null,u=u&&ot(o,a)}c=c||h&&s-d-1>r&&e[d+1]!==" "}return!l&&!c?u&&!i(e)?Kt:qt:t>9&&Yt(e)?Fe:c?zt:Vt}function vl(e,n,t,r){e.dump=function(){if(n.length===0)return"''";if(!e.noCompatMode&&pl.indexOf(n)!==-1)return"'"+n+"'";var i=e.indent*Math.max(1,t),s=e.lineWidth===-1?-1:Math.max(Math.min(e.lineWidth,40),e.lineWidth-i),o=r||e.flowLevel>-1&&t>=e.flowLevel;function a(l){return gl(e,l)}switch(_l(n,o,e.indent,s,a)){case Kt:return n;case qt:return"'"+n.replace(/'/g,"''")+"'";case Vt:return"|"+st(n,e.indent)+at(rt(n,i));case zt:return">"+st(n,e.indent)+at(rt(wl(n,s),i));case Fe:return'"'+kl(n)+'"';default:throw new Ae("impossible error: invalid scalar style")}}()}function st(e,n){var t=Yt(e)?String(n):"",r=e[e.length-1]===`
`,i=r&&(e[e.length-2]===`
`||e===`
`),s=i?"+":r?"":"-";return t+s+`
...
`+e.slice(o+1):l+=e.slice(i),l.slice(1)}function kl(e){for(var n="",t,r,i,s=0;s<e.length;s++){if(t=e.charCodeAt(s),t>=55296&&t<=56319&&(r=e.charCodeAt(s+1),r>=56320&&r<=57343)){n+=it((t-55296)*1024+r-56320+65536),s++;continue}i=O[t],n+=!i&&ce(t)?e[s]:i||it(t)}return n}function xl(e,n,t){var r="",i=e.tag,s,o;for(s=0,o=t.length;s<o;s+=1)ne(e,n,t[s],!1,!1)&&(s!==0&&(r+=","+(e.condenseFlow?"":" ")),r+=e.dump);e.tag=i,e.dump="["+r+"]"}function Cl(e,n,t,r){var i="",s=e.tag,o,a;for(o=0,a=t.length;o<a;o+=1)ne(e,n+1,t[o],!0,!0)&&((!r||o!==0)&&(i+=sn(e,n)),e.dump&&_e===e.dump.charCodeAt(0)?i+="-":i+="- ",i+=e.dump);e.tag=s,e.dump=i||"[]"}function Tl(e,n,t){var r="",i=e.tag,s=Object.keys(t),o,a,l,c,h;for(o=0,a=s.length;o<a;o+=1)h="",o!==0&&(h+=", "),e.condenseFlow&&(h+='"'),l=s[o],c=t[l],ne(e,n,l,!1,!1)&&(e.dump.length>1024&&(h+="? "),h+=e.dump+(e.condenseFlow?'"':"")+":"+(e.condenseFlow?"":" "),ne(e,n,c,!1,!1)&&(h+=e.dump,r+=h));e.tag=i,e.dump="{"+r+"}"}function Al(e,n,t,r){var i="",s=e.tag,o=Object.keys(t),a,l,c,h,d,u;if(e.sortKeys===!0)o.sort();else if(typeof e.sortKeys=="function")o.sort(e.sortKeys);else if(e.sortKeys)throw new Ae("sortKeys must be a boolean or a function");for(a=0,l=o.length;a<l;a+=1)u="",(!r||a!==0)&&(u+=sn(e,n)),c=o[a],h=t[c],ne(e,n+1,c,!0,!0,!0)&&(d=e.tag!==null&&e.tag!=="?"||e.dump&&e.dump.length>1024,d&&(e.dump&&_e===e.dump.charCodeAt(0)?u+="?":u+="? "),u+=e.dump,d&&(u+=sn(e,n)),ne(e,n+1,h,!0,d)&&(e.dump&&_e===e.dump.charCodeAt(0)?u+=":":u+=": ",u+=e.dump,i+=u));e.tag=s,e.dump=i||"{}"}function ct(e,n,t){var r,i,s,o,a,l;for(i=t?e.explicitTypes:e.implicitTypes,s=0,o=i.length;s<o;s+=1)if(a=i[s],(a.instanceOf||a.predicate)&&(!a.instanceOf||typeof n=="object"&&n instanceof a.instanceOf)&&(!a.predicate||a.predicate(n))){if(e.tag=t?a.tag:"?",a.represent){if(l=e.styleMap[a.tag]||a.defaultStyle,Dt.call(a.represent)==="[object Function]")r=a.represent(n,l);else if(Pt.call(a.represent,l))r=a.represent[l](n,l);else throw new Ae("!<"+a.tag+'> tag resolver accepts not "'+l+'" style');e.dump=r}return!0}return!1}function ne(e,n,t,r,i,s){e.tag=null,e.dump=t,ct(e,t,!1)||ct(e,t,!0);var o=Dt.call(e.dump);r&&(r=e.flowLevel<0||e.flowLevel>n);var a=o==="[object Object]"||o==="[object Array]",l,c;if(a&&(l=e.duplicates.indexOf(t),c=l!==-1),(e.tag!==null&&e.tag!=="?"||c||e.indent!==2&&n>0)&&(i=!1),c&&e.usedDuplicates[l])e.dump="*ref_"+l;else{if(a&&c&&!e.usedDuplicates[l]&&(e.usedDuplicates[l]=!0),o==="[object Object]")r&&Object.keys(e.dump).length!==0?(Al(e,n,e.dump,i),c&&(e.dump="&ref_"+l+e.dump)):(Tl(e,n,e.dump),c&&(e.dump="&ref_"+l+" "+e.dump));else if(o==="[object Array]"){var h=e.noArrayIndent&&n>0?n-1:n;r&&e.dump.length!==0?(Cl(e,h,e.dump,i),c&&(e.dump="&ref_"+l+e.dump)):(xl(e,h,e.dump),c&&(e.dump="&ref_"+l+" "+e.dump))}else if(o==="[object String]")e.tag!=="?"&&vl(e,e.dump,n,s);else{if(e.skipInvalid)return!1;throw new Ae("unacceptable kind of an object to dump "+o)}e.tag!==null&&e.tag!=="?"&&(e.dump="!<"+e.tag+"> "+e.dump)}return!0}function Sl(e,n){var t=[],r=[],i,s;for(an(e,t,r),i=0,s=r.length;i<s;i+=1)n.duplicates.push(t[r[i]]);n.usedDuplicates=new Array(s)}function an(e,n,t){var r,i,s;if(e!==null&&typeof e=="object")if(i=n.indexOf(e),i!==-1)t.indexOf(i)===-1&&t.push(i);else if(n.push(e),Array.isArray(e))for(i=0,s=e.length;i<s;i+=1)an(e[i],n,t);else for(r=Object.keys(e),i=0,s=r.length;i<s;i+=1)an(e[r[i]],n,t)}function Xt(e,n){n=n||{};var t=new ml(n);return t.noRefs||Sl(e,t),ne(t,0,e,!0,!0)?t.dump+`
`:""}function Il(e,n){return Xt(e,Te.extend({schema:Xa},n))}mn.dump=Xt;mn.safeDump=Il;var We=ke,Jt=mn;function je(e){return function(){throw new Error("Function "+e+" is deprecated and cannot be used.")}}T.Type=L;T.Schema=he;T.FAILSAFE_SCHEMA=dn;T.JSON_SCHEMA=bt;T.CORE_SCHEMA=_t;T.DEFAULT_SAFE_SCHEMA=Ce;T.DEFAULT_FULL_SCHEMA=$e;T.load=We.load;T.loadAll=We.loadAll;T.safeLoad=We.safeLoad;T.safeLoadAll=We.safeLoadAll;T.dump=Jt.dump;T.safeDump=Jt.safeDump;T.YAMLException=xe;T.MINIMAL_SCHEMA=dn;T.SAFE_SCHEMA=Ce;T.DEFAULT_SCHEMA=$e;T.scan=je("scan");T.parse=je("parse");T.compose=je("compose");T.addConstructor=je("addConstructor");var El=T,Ml=El;function Ll(e){if(!e.startsWith(`---
`))return{data:{},content:e};const n=e.indexOf(`
---`,4);if(n===-1)return{data:{},content:e};const t=e.slice(4,n),r=e.slice(n+4),i=r.startsWith(`
`)?r.slice(1):r;return{data:Ml.safeLoad(t)??{},content:i}}function Rl(e){const n={settings:{player:{name:"Captain",startingCredits:0},startingLocation:{system:"",destination:""},startingShip:""},systems:[],destinations:[],routes:[],drives:[],ships:[],factions:[],commodities:[],storyBeats:[],deliveryItems:[],npcNames:{special:[],firstNames:[],lastNames:[]}};for(const[t,r]of Object.entries(e)){const i=t.split("/").pop()??"";if(i==="_template.md"||i===".gitkeep")continue;const{data:s,content:o}=Ll(r);/^systems\/[^/]+\.md$/.test(t)?n.systems.push(Ol(s,o)):/^destinations\/[^/]+\.md$/.test(t)?n.destinations.push(Nl(s,o)):/^factions\/[^/]+\.md$/.test(t)?n.factions.push(Fl(s,o)):/^ships\/[^/]+\.md$/.test(t)?n.ships.push(Dl(s,o)):t==="ships/components/jump-drives.md"?n.drives=Pl(s):t==="navigation/jump-routes.md"?n.routes=Ul(s):t==="commodities.md"?n.commodities=Bl(s):/^story\/[^/]+\.md$/.test(t)?n.storyBeats.push(Hl(s,o)):t==="game-settings.md"?n.settings=$l(s):t==="delivery-items.md"?n.deliveryItems=Gl(s):t==="npc-names.md"&&(n.npcNames=Wl(s))}return n}function Ye(e){const n=[];let t=!1;for(const r of e.split(`
`)){const i=r.trim();if(!i.startsWith("#"))if(i===""){if(t)break}else t=!0,n.push(i)}return n.join(" ")}function Ol(e,n){return{id:e.id,name:e.name,starType:e.star_type,distanceFromSol:e.distance_from_sol,zone:e.zone,security:e.security,population:e.population,dangerLevel:e.danger_level,playerKnowledge:e.player_knowledge,economy:e.economy??[],majorFactions:e.major_factions??[],destinations:e.destinations??[],tags:e.tags??[],description:Ye(n)}}function Nl(e,n){const t=e.amenities??{},r={trader:t.trader??!1,missionBoard:t.mission_board??!1,shipRepair:t.ship_repair??!1,fuel:t.fuel??!1,shipDealer:t.ship_dealer??!1};return{id:e.id,name:e.name,system:e.system,locationType:e.location_type,type:e.type,amenities:r,npcs:e.npcs??{},goodsBias:e.goods_bias??[],dangerLevel:e.danger_level,tags:e.tags??[],description:Ye(n)}}function Fl(e,n){return{id:e.id,name:e.name,type:e.type,homeSystem:e.home_system,size:e.size,influence:e.influence??[],tags:e.tags??[],description:Ye(n)}}function Dl(e,n){return{id:e.id,name:e.name,class:e.class,cost:e.cost,cargoCapacityKg:e.cargo_capacity_kg,fuelCapacityL:e.fuel_capacity_l,hullPoints:e.hull_points,defaultJumpDrive:e.default_jump_drive,tags:e.tags??[],description:Ye(n)}}function Pl(e){return(e.drives??[]).map(t=>({id:t.id,name:t.name,maxDistanceLy:t.max_distance_ly,fuelEfficiency:t.fuel_efficiency,cost:t.cost}))}function Ul(e){return(e.routes??[]).map(t=>({from:t.from,to:t.to,distance:t.distance,stability:t.stability,security:t.security}))}function Bl(e){return(e.commodities??[]).map(t=>({id:t.id,name:t.name,basePrice:t.base_price,category:t.category,legal:t.legal,weightKg:t.weight_kg,description:t.description??""}))}function Hl(e,n){return{id:e.id,title:e.title,trigger:e.trigger,type:e.type,location:e.location,skippable:e.skippable,playerKnowledge:e.player_knowledge,text:n.trim()}}function $l(e){var n,t,r,i;return{player:{name:((n=e.player)==null?void 0:n.name)??"Captain",startingCredits:((t=e.player)==null?void 0:t.starting_credits)??0},startingLocation:{system:((r=e.starting_location)==null?void 0:r.system)??"",destination:((i=e.starting_location)==null?void 0:i.destination)??""},startingShip:e.starting_ship??""}}function Gl(e){return(e.delivery_items??[]).map(t=>({id:t.id,name:t.name,weightKg:t.weight_kg}))}function Wl(e){const n=e.npc_names??{};return{special:n.special??[],firstNames:n.first_names??[],lastNames:n.last_names??[]}}function jl(){const e=Object.assign({"/docs/world/commodities.md":Dr,"/docs/world/delivery-items.md":Pr,"/docs/world/destinations/_template.md":Ur,"/docs/world/destinations/blackwake-yard.md":Br,"/docs/world/destinations/ceti-landfall.md":Hr,"/docs/world/destinations/drift-market.md":$r,"/docs/world/destinations/elysium-station.md":Gr,"/docs/world/destinations/eridani-anchorage.md":Wr,"/docs/world/destinations/foundries-platform.md":jr,"/docs/world/destinations/galileo-transfer.md":Yr,"/docs/world/destinations/hestia-ring.md":Kr,"/docs/world/destinations/keelhaul-station.md":qr,"/docs/world/destinations/kepler-yard.md":Vr,"/docs/world/destinations/mars-anchor.md":zr,"/docs/world/destinations/meridian-station.md":Xr,"/docs/world/destinations/new-horizon-port.md":Jr,"/docs/world/destinations/orrery-anchorage.md":Qr,"/docs/world/destinations/redline-station.md":Zr,"/docs/world/destinations/tycho-orbital.md":eo,"/docs/world/destinations/veil-station.md":no,"/docs/world/destinations/waypoint-ceti.md":to,"/docs/world/factions/_template.md":io,"/docs/world/factions/centauri-trade-league.md":ro,"/docs/world/factions/eridani-colonial-council.md":oo,"/docs/world/factions/free-captains.md":so,"/docs/world/factions/grey-market-cartel.md":ao,"/docs/world/factions/helios-directorate.md":lo,"/docs/world/factions/independent-miners-guild.md":co,"/docs/world/factions/procyon-institute.md":ho,"/docs/world/factions/terran-union.md":uo,"/docs/world/galaxy-map.md":po,"/docs/world/game-settings.md":fo,"/docs/world/navigation/jump-routes.md":mo,"/docs/world/npc-names.md":go,"/docs/world/ships/_template.md":yo,"/docs/world/ships/components/jump-drives.md":bo,"/docs/world/ships/freighter.md":_o,"/docs/world/ships/hauler.md":vo,"/docs/world/ships/scout.md":wo,"/docs/world/story/_template.md":ko,"/docs/world/story/enter-wolf-359.md":xo,"/docs/world/story/first-jump.md":Co,"/docs/world/story/opening-arrival.md":To,"/docs/world/systems/_template.md":Ao,"/docs/world/systems/alpha-centauri.md":So,"/docs/world/systems/barnards-star.md":Io,"/docs/world/systems/epsilon-eridani.md":Eo,"/docs/world/systems/procyon.md":Mo,"/docs/world/systems/sirius.md":Lo,"/docs/world/systems/sol.md":Ro,"/docs/world/systems/tau-ceti.md":Oo,"/docs/world/systems/wolf-359.md":No}),n={};for(const[t,r]of Object.entries(e)){const i=t.replace("/docs/world/","");n[i]=r}return Rl(n)}ui(jl());const Yl=navigator.maxTouchPoints>0?"touch":"keyboard",Kl=new URLSearchParams(window.location.search).has("debug"),Qt={environment:"browser",primaryInput:Yl,debug:Kl},ql=new ii,Zt=new ai(Qt);Zt.connect();const Vl=new Fr(ql,Zt,Qt);let ht=0;function ei(e){Vl.tick(e-ht),ht=e,requestAnimationFrame(ei)}requestAnimationFrame(ei);
