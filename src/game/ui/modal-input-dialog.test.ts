...
function makeForm(overrides: Partial<ModalFormDef> = {}): ModalFormDef {
  return {
    title: 'BUY IRON ORE',
    field: { label: 'Quantity', initialValue: 3, min: 0, max: 10, step: 1 },
...
describe('ModalInputDialog — UP/DOWN adjust value', () => {
  it('UP increments value by step', () => {
    const dialog = new ModalInputDialog(makeForm());
  ...
  it('DOWN decrements value by step', () => {
    const dialog = new ModalInputDialog(makeForm());
  ...
  it('UP clamps at max', () => {
    const dialog = new ModalInputDialog(makeForm({ field: { label: 'Quantity', initialValue: 10, min: 0, max: 10, step: 1 } }));
    dialog.handleAction('UP');
    const buf = makeBuffer(BUF_W, BUF_H);
    dialog.render(buf);
  ...
  it('DOWN clamps at min (0)', () => {
    const dialog = new ModalInputDialog(makeForm({ field: { label: 'Quantity', initialValue: 0, min: 0, max: 10, step: 1 } }));
    dialog.handleAction('DOWN');
    const buf = makeBuffer(BUF_W, BUF_H);
    dialog.render(buf);
    // value stays 0
...
describe('ModalInputDialog — char input', () => {
  ...
  it('digit input is clamped at max', () => {
    const dialog = new ModalInputDialog(makeForm({ field: { label: 'Quantity', initialValue: 3, min: 0, max: 5, step: 1 } }));
  ...
  it('backspace to zero resets replaceNextDigit so next digit replaces', () => {
    const dialog = new ModalInputDialog(makeForm({ field: { label: 'Quantity', initialValue: 3, min: 0, max: 10, step: 1 } }));
...
describe('ModalInputDialog — action routing', () => {
  ...
  it('LEFT on cancel moves to confirm', () => {
    const onConfirm = vi.fn();
    const dialog = new ModalInputDialog(makeForm({ onConfirm }));
    dialog.handleAction('TAB'); // → confirm
    dialog.handleAction('TAB'); // → cancel
    dialog.handleAction('LEFT'); // → confirm
    dialog.handleAction('SELECT');
    expect(onConfirm).toHaveBeenCalled();
  });

  it('UP on cancel moves to field', () => {
    const dialog = new ModalInputDialog(makeForm());
    dialog.handleAction('TAB'); // → confirm
    dialog.handleAction('TAB'); // → cancel
    dialog.handleAction('UP');  // → field
    const buf = makeBuffer(BUF_W, BUF_H);
    dialog.render(buf);
    const valueBoxCol = DIALOG_COL + 1 + 8 + 3;
    expect(cellBg(buf, DIALOG_ROW + 4, valueBoxCol)).toBe('green');
...
describe('ModalInputDialog — zero derived rows', () => {
  it('renders with no derived rows; height is 8', () => {
    const dialog = new ModalInputDialog({
      title: 'TEST',
      field: { label: 'Val', initialValue: 1, min: 0, max: 9, step: 1 },
      derivedRows: [],
      confirmLabel: 'OK',
      onConfirm: vi.fn(),
      onCancel: vi.fn(),
