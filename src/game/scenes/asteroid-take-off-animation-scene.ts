...
export class AsteroidTakeOffAnimationScene extends BaseTransitionScene {
  ...
  protected override renderContent(buffer: CharBuffer, _top: number, _bottom: number): void {
    ...
    writeCentered(buffer, mid - 3, '[ RELEASING CLAMPS ]', 'bright-yellow', 'black');
    writeCentered(buffer, mid + 2, RELEASE_FRAMES[frameIdx], 'bright-black', 'black');
    writeCentered(buffer, mid + 4, `DEPARTING IN ${countdown}S`, 'bright-black', 'black');
  }
}
