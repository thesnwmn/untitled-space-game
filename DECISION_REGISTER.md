...
| `docs/decisions/travel-system.md` | `TravelMenuScene` constructor and tab layout; in-space `ShipScene` state; `currentDestinationId` as single source of truth; all six animation scene durations, chrome overrides, and routing |
...
| Separate animation scenes | One class per transition (six total) | Distinct classes preserve the option to diverge visually without conditional branching |
