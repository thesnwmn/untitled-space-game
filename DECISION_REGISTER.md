...
| Scene base | `BaseScene` abstract class | Centralises buffer clearing, `ScreenChrome`, title/underline, summary lines, tab bar, and all input routing; all scenes except `MainMenuScene` extend it directly or transitively |
| Transition scene base | `BaseTransitionScene` abstract class | Extends `BaseScene`; centralises timing guard and auto-advance; subclasses implement only `renderContent(buffer, top, bottom)` and optionally override `buildChromeConfig()` |
