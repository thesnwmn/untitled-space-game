...
### Reviewer
Activated when: the manager asks you to review a completed backlog item.
...
4. Read any existing code that the feature touches.
5. Implement. Follow DECISION_REGISTER.md — do not introduce new dependencies or
...
7. Run `init.sh` again. It must pass clean on the finished state.
8. Update BACKLOG.md:
   - Move the item to DONE.
   - Record: what was built, tsc output, test results, and play-test instructions.
9. If you run out of context before finishing:
...
6. Return one of:
   - **Approved** — with any minor observations noted (non-blocking).
   - **Issues found** — list specific problems. Do not fix them yourself; flag for
     the Engineer or Debugger to address.
